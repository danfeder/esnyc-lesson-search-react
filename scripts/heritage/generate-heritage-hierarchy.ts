 ...
 *
 * Ordering rule (explicit, deterministic, applied uniformly to roots and to
 * every sibling group): vocab `frequency` DESCENDING, then `key` ASCENDING as
 * a stable tie-break. Re-running the generator therefore produces byte-
 * identical output — the drift guard test asserts exactly that.
 *
...
// ---------------------------------------------------------------------------
// Ordering rule: frequency DESC, then key ASC (stable tie-break)
// ---------------------------------------------------------------------------
...
export function renderSeedSql(vocab: HeritageVocab): string {
  const rows = vocab.canonical.slice().sort(compareNodes);
  const values = rows
    .map((n) => {
