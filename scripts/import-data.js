...
async function importLessons() {
  try {
    ...
    const transformedLessons = lessons.map(lesson => ({
      ...
      confidence: lesson.confidence || { overall: 0.5, title: 0.5, summary: 0.5, gradeLevels: 0.5 }
    })).filter(lesson => lesson.lesson_id && lesson.file_link); // Filter out lessons without essential data
    
    console.log(`📋 Filtered to ${transformedLessons.length} valid lessons (removed ${lessons.length - transformedLessons.length} incomplete entries)`);
    ...
    for (let i = 0; i < transformedLessons.length; i += batchSize) {
      ...
      console.log(`📥 Importing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(transformedLessons.length / batchSize)}...`);
