...
**Last updated:** 2026-05-12 — Session 76 PR #492 (§16 end-summary parser-driven regeneration) squash-merged 2026-05-12 as `e5f4257` (closeout backfill direct-to-main per Sessions 67 + 68/PR #485 + 69 + 71 + 73 + 74 + 75 precedent — 8th occurrence; user-authorized). Path (a) parser-driven over (b) hand-regen at session start (user choice): new `scripts/parse-heritage-worksheet.py` (441-line Python; first Python utility in this repo) reads labeled-line per-value entries per §4 + §7 conventions and emits the §16 88-row 7-column markdown table; reusable for future Stage 1 / Stage 2 worksheets. §16 verdict cells filled (no `<to_fill>` remaining); preamble references the parser and records final distributions. 3 bot-review rounds: **Round 1** = 3-voice (claude-review + user-dispatched Codex + Hermes) → 4 substantive accepts in fix-up `55d8510` (F1 Hermes P1 `--apply` destructive-write gate + F2 3-voice P2 `encoding="utf-8"` on 3 I/O sites + F3 Codex P3 `verify_invariants` whitelist + distribution assertions + F7 cosmetic `parse_value` return-type narrow), 3 rejects (F5 filename casing / F6 `--apply` double-read / F8 entry-boundary heuristic — rebuttals posted). **Round 2** = single-voice claude → 1 cosmetic accept in fix-up `f66b992` (R2-2 `_CLUSTER_ORDER` module-level hoist), 6 rejects (4 repeats from prior rounds + R2-3 ergonomic file-not-found + R2-7 missing pytest). **Round 3** = single-voice claude → 0 accepts per round-cap rule ("3rd round = critical-bugs-only"); all 6 findings default-rejected (4 repeats + R3-3 cosmetic + R3-4 bot misread of the `and "<cluster>" in line` AND-condition). Pre-push code-reviewer agent on round-1 fix-up returned 0 P1/P2/P3 findings. Security Audit pre-existing fail default-rejected as out-of-scope per repo hygiene-follow-up. **Stage 1 worksheet is now fully complete — content + scaffolding + structural §16 regeneration all ✅ Shipped — awaiting downstream consumption (PR 5+ D4 canonicalization migration, PR 6+ Stage 2 corpus re-tag).** Foundation-phase code track still has no unblocked next PR — PR 3b / 5 / 6 all gate on Stage 1 / Stage 2 outputs.
...
| Cross-cluster §9 (§9.1) | ✅ Shipped | #489 | `3ea3ecb` | 13 per-value entries: 1 cluster root NEW + African American 24 + African American diaspora 2 + Indigenous 24 + Native American 5 + Indigenous/Native American 1 + Indigenous Peoples 1 + Lenape 7 + Haudenosaunee 3 NEW + 4 v3-corpus-absent (Soul Food, Three Sisters traditions, Black culinary history, Cajun/Creole); 2 Opus corpus-read `<details>` blocks (African American + Indigenous); 9 new cross-cluster X-NN audit signals (X-01 through X-09); §9.2 multi-parent table unchanged (5/5 complete from Session 70); §9.3 + §9.4 housekeeping unchanged |
| End summary §16 | ✅ Shipped | #490 | `d47ed8d` | 88 rows mechanically transcribed (Asian 18 + Americas 22 + African 10 + European 14 + Middle Eastern 11 + Cross-cluster §9.1 13); tier distribution 12 top + 19 sub + 57 internal; merge target inline encoded in `verdict` cell as `merge → <slug>` per §7 alias_map identity convention; `<to_fill>` cells gated on §9.1 Decisions 1 + 3; final substantive Stage 1 worksheet work pre-curriculum-team-handoff |
| Curriculum-team fill integration | ✅ Shipped | #491 | `d8af791` | Curriculum-team-returned worksheet integrated: §9.1 framing decisions D1-D5 answered ("Indigenous and Diaspora" cluster root / `African American` canonical / `Indigenous` canonical / tribal-nations parented under `indigenous` / v3-corpus-absent → `internal`-tier placeholders); 88 per-value verdicts filled; 4 §9.1 merge entries parented under `indigenous-and-diaspora`; D5 wording aligned with verdicts; 18 NEW-candidate sub-regions and country-specifics verdict-flipped to `new` per §3 spec across round-1 + round-2 fix-ups. Final counts 51 keep / 17 merge / 20 new |
| End summary §16 regeneration | ✅ Shipped | #492 | `e5f4257` | Parser-driven path (a): new `scripts/parse-heritage-worksheet.py` (441-line Python; first Python utility in repo) reads labeled-line per-value entries per §4 + §7 conventions and emits the 88-row 7-column markdown table; reusable for Stage 1 / Stage 2 worksheets. §16 verdict cells fully filled (no `<to_fill>` remaining); preamble references the parser + records final distributions. 3 fix-up rounds: round-1 4-accept (F1 Hermes P1 `--apply` gate + F2 3-voice P2 UTF-8 + F3 Codex P3 whitelist + distribution assertions + F7 cosmetic) in `55d8510`, round-2 1-accept (R2-2 `_CLUSTER_ORDER` hoist) in `f66b992`, round-3 0-accept per round-cap |

Audit signal register (Stage 2 corpus cleanup / reviewer-validation intake): `docs/plans/2026-05-10-metadata-rebuild-stage1-heritage-audit-signal-register.md`. Carries the audit signals surfaced during Stage 1 per-value fills (~50 open as of 2026-05-12 post-Session-76: 4 ASI + 6 AME + 8 AFR + 14 EUR + 9 ME + 9 X — Sessions 74-76 added 0 audit signals since both §16 transcription and parser-driven regeneration are mechanical operations on the now-frozen per-value entries). Covers re-tag actions, body-content review, and reviewer-led judgment calls. Append new signals there; do not duplicate into per-cluster status-doc narrative.
...
PR #492 closeout marks (Session 76; refresh per-PR at next cycle):

- [x] Record PR number and squash commit in the dashboard row for the shipped artifact *(Session 76 PR #492 → squash `e5f4257`, 2026-05-12)*
- [x] Update Current state dashboard row (status `✅ Shipped`, PR number, merge commit, notes summary) *(New "End summary §16 regeneration" row → ✅ Shipped | #492 | `e5f4257`)*
- [x] Update `Last updated` line in this doc *(Session 76 closeout banner — 3-round bot review cycle summarized; round-cap rule applied at round 3)*
- [x] Update foundation status doc pointer (Current State header + PRs-SHIPPED list) *(PR #492 added to PRs-SHIPPED list)*
- [x] Update Branches block in foundation status doc (move branch from "Active" to traceability list) *(docs/stage1-heritage-section-16-regen squash-merged + auto-deleted on origin via `--delete-branch`; local auto-switched to main on merge)*
- [x] Update Next session contract for the next session (session number, branch base, primary objective, expected files, first task, must verify, do not do) *(Session 77 TBD by user direction — Stage 1 has no inherent next step now that content + scaffolding + structural §16 regeneration are all shipped)*
- [x] For status-tracking / hygiene PRs, update the Next session contract branch base to this PR's squash commit after merge *(N/A — tooling-track PR, not hygiene; branch base for any Stage 1 follow-up would be the Session 76 closeout-backfill commit)*
- [x] Append new audit signals to `docs/plans/2026-05-10-metadata-rebuild-stage1-heritage-audit-signal-register.md` *(N/A — parser-driven §16 regeneration is a mechanical operation on the now-frozen per-value entries; no new corpus reads, no new audit signals surfaced; cluster total stays at 50)*
- [x] Search for stale strings and replace:
  ...
  *(N/A — Session 76 had no in-flight stale strings; the parser-regen commit replaced all §16 `<to_fill>` verdict cells with actual verdicts pre-PR)*
- [x] Confirm no migration / package / Supabase / test files changed in the closing PR *(Verified at ship via `git diff --name-only origin/main...HEAD` → 3 files: `.gitignore` + `docs/plans/...worksheet.md` + `scripts/parse-heritage-worksheet.py`. The script in `scripts/` is the path-(a) parser-driven regeneration tool explicitly scoped in the Session 76 Next session contract; not strictly docs-only but in-scope for this session)*
...
**End summary §16 ✅ Session 74 (2026-05-12). Branch `docs/stage1-heritage-end-summary` off `main` at `52c633a` (Session 73 PR #489 closeout-backfill commit, downstream of PR #489 squash `3ea3ecb`); pre-push code-reviewer agent ran a full 616-cell verification (88 rows × 7 columns) cross-referenced against every per-value entry in §11-§15 + §9.1, plus structural checks (row count, tier distribution, orphan parent, drift conventions) and status doc accuracy checks — returned 0 findings. Mechanical-transcription PR was clean; the 13-session Stage 1 catch-streak (Sessions 60-73) ends at Session 74 with 0 substantive findings (the streak's natural endpoint for a mechanical-transcription pattern with no body-text Opus reads or framing decisions). PR #490 squash-merged 2026-05-12 00:05 UTC as `d47ed8d`. Round-1 claude-review bot returned "Approve with minor notes" — 5 observations all default-rejected with rationale captured in PR comment (each finding traced to pre-existing per-value Notes content faithfully transcribed into §16, not §16-introduced).** Worksheet §16 end-summary canonical-vocab table populated with all 88 rows: Asian §11 (15 substantive + 3 drift = 18) + Americas §12 (18 substantive + 4 drift = 22) + African §13 (9 substantive + 1 drift = 10) + European §14 (11 substantive + 3 drift = 14) + Middle Eastern §15 (9 substantive + 2 drift = 11) + Cross-cluster §9.1 (13, all substantive). Tier distribution: 12 top + 19 sub + 57 internal. Merge target encoded inline in the `verdict` cell as `merge → <slug>` to honor the 7-column spec while preserving the merge relationship for human readers; structured `merge_into` stays in per-value entries per §7 parsing convention. `<to_fill>` cells gated on §9.1 Decision 1 (cluster root canonical_key + surface_label) + Decision 3 (Indigenous canonical_key + surface_label) + downstream parent reroutes — 2 of 88 rows carry `<to_fill>` for canonical_key + surface_label; all other 86 rows have all 6 structural fields populated. **The §16 table is the hand-off artifact to PR 5+ (D4 vocab canonicalization migration) and PR 6+ (Stage 2 re-tag prompt closed-vocab constraint).** Foundation-phase code track still has no unblocked next PR — PR 3b / 5 / 6 all gate on Stage 1 / Stage 2 outputs.

**All 5 regional clusters (§11-§15) + cross-cluster §9.1 + §16 end-summary canonical-vocab table now ✅ Shipped — the worksheet is substantively complete pre-curriculum-team-handoff.**

**Curriculum-team fill ✅ integrated to `main` (Session 75 PR #491 squash `d8af791`, 2026-05-12).** 88 per-value verdicts filled (51 keep / 17 merge / 20 new); §9.1 framing decisions D1-D5 answered ("Indigenous and Diaspora" cluster root, `African American` canonical, `Indigenous` canonical, tribal-nations parented under `indigenous`, v3-corpus-absent → `internal`-tier placeholders); 4 §9.1 merge entries parented under `indigenous-and-diaspora`; D5 wording aligned with actual verdicts; 18 NEW-candidate sub-regions and country-specifics verdict-flipped to `new` per §3 spec across round-1 + round-2 fix-ups (14 in round-1 `ce74ea6` covering north-african / east-african / central-american / south-american / southern-united-states / honduran / cuban / kenyan / egyptian / moroccan / irish / yemeni / persian / sri-lankan, +4 in round-2 `7c092da` covering peruvian / brazilian / ecuadorian / guyanese closing the round-1 list gap). 3 claude-review rounds completed; final round-3 bot verdict "Ship-ready." **Stage 1 content track substantively complete after PR #491; remaining structural gap was §16 end-summary regeneration — closed by Session 76 (below).**

**§16 end-summary parser-driven regeneration ✅ shipped Session 76 (PR #492 squash `e5f4257`, 2026-05-12).** Path (a) parser-driven over (b) hand-regen at session start (user choice): new `scripts/parse-heritage-worksheet.py` (441-line Python; first Python utility in repo) reads labeled-line per-value entries per §4 + §7 conventions and emits the 88-row 7-column markdown table; reusable for future Stage 1 / Stage 2 worksheets. §16 verdict cells fully filled (no `<to_fill>` remaining); preamble references the parser + records final distributions (51 keep / 17 merge / 20 new = 88; 12 top / 19 sub / 57 internal = 88). 3 bot-review rounds: **Round 1** = 3-voice (claude-review + user-dispatched Codex + Hermes) → 4 substantive accepts in fix-up `55d8510` (F1 Hermes P1 `--apply` destructive-write gate when `verify_invariants` returns problems + F2 3-voice P2 `encoding="utf-8"` on 3 I/O sites + F3 Codex P3 `verify_invariants` whitelist `{top, sub, internal}` / `{keep, merge, new, <to_fill>}` plus expected-distribution assertions + F7 cosmetic `parse_value` return-type narrow `object → str | None | list[str]`), 3 rejects (F5 filename casing / F6 `--apply` double-read / F8 entry-boundary heuristic — rebuttals posted to the PR). **Round 2** = single-voice claude → 1 cosmetic accept in fix-up `f66b992` (R2-2 `_CLUSTER_ORDER` module-level hoist), 6 rejects (R2-1 hardcoded-counts CLI flag = repeat; R2-3 raw `FileNotFoundError` = cosmetic ergonomics; R2-4 `setattr` type-bypass = repeat; R2-5 `--apply` fence regex = repeat; R2-6 `--verify-only` stderr-only = intentional design; R2-7 missing pytest = memory-tracked follow-up). **Round 3** = single-voice claude → 0 accepts per kickoff round-cap rule ("3rd round = critical-bugs-only"); all 6 findings default-rejected (4 repeats from prior rounds + R3-3 cosmetic friendlier-FileNotFoundError + R3-4 bot misread of the `if "<" in rest and "<cluster>" in line` AND-condition — current heuristic is correctly layered). Pre-push code-reviewer agent on round-1 fix-up returned 0 P1/P2/P3 findings with "Approve as-is." Negative-path verification probes for F1 / F2 / F3 all reproduce pre-fix bug + pass post-fix; positive: `--verify-only` exit=0 + `--apply` idempotent. Security Audit pre-existing fail (`@lhci/cli` chain) default-rejected as out-of-scope per repo hygiene-follow-up. **Stage 1 worksheet is now fully complete — content + scaffolding + structural §16 regeneration all ✅ Shipped. Stage 1 has no inherent next step; Session 77 awaits user-pivot direction.**

---

**Prior cluster history retained (Session 72-73 → Session 73 closeout):** Cross-cluster §9.1 Diaspora & Indigenous Identities ✅ Session 72-73 (2026-05-11). Branch `docs/stage1-heritage-cross-cluster-and-end-summary` off `main` at `4b34aa0` (Session 71 PR #488 closeout-backfill commit; pre-push agent caught 2 P2 + 1 P3 + fix-up `84201aa`; Session 73 round-1 fix-ups `3e45b5c` + round-2 PR-body-only resolution; PR #489 squash-merged 2026-05-11 23:34 UTC as `3ea3ecb`). Worksheet §9.1 has all 13 per-value entries populated: **9.1.1** Diaspora & Indigenous (cluster root — NEW canonical, 57 distinct aggregate child-value lessons) + **9.1.2** African American (24, with `<details>` corpus-evidence block) + **9.1.3** African American diaspora (2, both problem rows — AFR-06 + X-05) + **9.1.4** Indigenous (24, with `<details>` corpus-evidence block) + **9.1.5** Native American (5, merge candidate per Decision 3) + **9.1.6** Indigenous/Native American (1, v3 canonical ghost row per X-08) + **9.1.7** Indigenous Peoples (1, merge into Indigenous) + **9.1.8** Lenape (7, keep + parent question Decision 4) + **9.1.9** Haudenosaunee (3, NEW canonical + parent question Decision 4) + **9.1.10** Soul Food (0, v3-corpus-absent per §12.18 Dominican precedent) + **9.1.11** Three Sisters traditions (0, v3-corpus-absent + X-09 cross-canonical-theme reframe) + **9.1.12** Black culinary history (0, v3-corpus-absent + likely-redundant-with-AA observation) + **9.1.13** Cajun/Creole (0, v3-corpus-absent + cross-ref §12.17 Southern United States NEW candidate). 2 substantive Opus corpus-read `<details>` blocks integrated (§9.1.2 African American 26-lesson cohort + §9.1.4 Indigenous 31-distinct-lesson cohort spanning 6 canonical-value surface labels — `Indigenous` 24 + `Native American` 5 + `Indigenous/Native American` 1 + `Indigenous Peoples` 1 + `Lenape` 7 + `Haudenosaunee` 3); the remaining 11 entries are structural per §4 methodology with direct SQL inspection of representative bodies. §9.2 multi-parent table unchanged (5/5 complete from Session 70). §9.3 + §9.4 cross-cluster housekeeping unchanged (no exceptions surfaced; §9.1 per-value entries follow §6 conventions with documented rationales). **9 cross-cluster `X-NN` audit signals appended to register (X-01 through X-09 — first cross-cluster signals in the register):** X-01 berbere `East African` omission on BHM lesson; X-02 4 Cajun BEP Slider rows uniformly omit `North American`; X-03 AA-cohort continental-`African` parent split (15 carry, 9 omit — convention question); X-04 `African American diaspora` v3-canonical functionally vacant; X-05 Soul Food Sunday template-stub published row; X-06 The Lenape Farmers and Skits missing `Lenape` tag; X-07 cross-cluster Indigenous-without-North-American routing (5 lessons via Latin American); X-08 `Indigenous/Native American` v3-canonical propagated to only 1 row; X-09 Three Sisters as cross-canonical-value theme rather than v3-baseline single-parent child. Cluster total open audit signals now 50 (4 ASI + 6 AME + 8 AFR + 14 EUR + 9 ME + 9 X). **§9.1 framing decisions surface but defer to curriculum-team verdicts (5 questions):** Decision 1 (cluster naming — single root `Diaspora & Indigenous` recommended pre-handoff vs split `Diaspora` + `Indigenous`); Decision 2 (`African American` vs `African American diaspora` surface label — empirically settled toward `African American` per X-04 + X-05); Decision 3 (`Indigenous` vs `Native American` vs `Indigenous/Native American` surface label — corpus-prevalence settles toward `Indigenous`); Decision 4 (Lenape + Haudenosaunee tier + parent — `Eastern Woodlands` NEW sub-region intermediate candidate); Decision 5 (v3-corpus-absent placeholder verdicts for Soul Food, Three Sisters traditions, Black culinary history, Cajun/Creole — Three Sisters is the X-09 reshape candidate). Pre-push code-reviewer agent caught 2 P2 + 1 P3 internal-consistency findings; all 3 accepted + fix-up'd in commit `84201aa` (thirteenth consecutive Stage 1 catch). **All 5 regional clusters (§11-§15) + cross-cluster §9.1 now ✅ have per-value entries populated.** §16 end-summary canonical-vocab table deferred to Session 74 per kickoff-prompt allowance ("Acceptable to ship §9.1 + §16 as 2 separate PRs if needed"). See Stage 1 dashboard above for per-cluster PR + merge commit traceability; see PRs-SHIPPED list in foundation status doc for full per-cluster decisions roll-up.
...
- **§16 end-summary canonical-vocab table: 88 rows mechanically transcribed ✅ (Session 74)** — tier distribution 12 top + 19 sub + 57 internal; merge target inline encoded as `merge → <slug>`; final substantive Stage 1 worksheet work pre-curriculum-team-handoff
- **Curriculum-team fill integration: 88 verdicts + D1-D5 ✅ (Session 75, PR #491 → `d8af791`)** — 51 keep / 17 merge / 20 new across all per-value entries; D1-D5 framing decisions answered; 18 NEW-candidate verdicts spec-corrected during 3-round bot review cycle
- **§16 end-summary parser-driven regeneration: ✅ (Session 76, PR #492 → `e5f4257`)** — new `scripts/parse-heritage-worksheet.py` (path (a)); 88 row verdicts regenerated from per-value entries; final distributions 51 keep / 17 merge / 20 new + 12 top / 19 sub / 57 internal; 3 fix-up rounds (round 1 4-accept, round 2 1-accept cosmetic, round 3 0-accept per round-cap)
...
### Session 76 — 2026-05-12 — §16 end-summary parser-driven regeneration + PR #492 ship cycle

**Branch:** `docs/stage1-heritage-section-16-regen` (off `main` at `e3287ae` — the Session 75 → Session 76 scope-lock direct-to-main closeout-backfill commit, downstream of PR #491 squash `d8af791`). PR #492 squash-merged 2026-05-12 as `e5f4257`; origin auto-deleted via `--delete-branch`.

**Done:**

- **Path-(a) chosen over path-(b).** Session opened with kickoff `Next session contract` offering two implementation paths: (a) parser-driven Python script per §4 + §7, or (b) one-off hand-regeneration. User chose (a) at session start for the long-term reuse benefit (Stage 2 worksheets are anticipated to follow the same labeled-line per-value entry shape; a reusable parser amortizes across them).
- **Built `scripts/parse-heritage-worksheet.py` (441 lines)** — first Python utility in the repo. Reads §11.X / §12.X / §13.X / §14.X / §15.X / §9.1.X per-value entries per §4 entry-shape convention; tolerates inline HTML comments per §7; handles 4 documented frequency-extraction shapes ("Asian (63)" / "`asian` (drift literal — 1 corpus appearance)" / "Indigenous/Native American (1, v3 canonical surface label)" / "Diaspora & Indigenous (cluster root — NEW canonical, 57 distinct ...)"). Emits 7-column markdown table (canonical_key | surface_label | parent | filter_ui_tier | frequency | aliases | verdict) with merge target encoded inline as `merge → <target_canonical_key>` to honor the 7-column spec while preserving the merge relationship for human readers per §7 alias_map convention. CLI: `--verify-only` (invariant checks, exit 0 if clean), `--apply` (in-place §16 fenced-block replacement), default (print to stdout for diff review). `__pycache__/` + `*.pyc` added to `.gitignore`.
- **§16 verdict cells fully filled.** Pre-Session-76 §16 carried `<to_fill>` verdict cells throughout (per "Do not hand-edit — mechanically regenerated" rule from §7). Parser ran against the now-filled per-value entries and emitted the table with all 88 verdicts populated. §9.1 cluster root canonical_key (`indigenous-and-diaspora`) + Indigenous canonical_key (`indigenous`) + parent chains all reflected per Session 75 curriculum-team fill (D1-D5 decisions).
- **Final distributions verified:** 88 rows total; 51 keep / 17 merge / 20 new (matches PR #491 verdict counts); 12 top / 19 sub / 57 internal (matches Session 74 tier distribution).
- **3 bot-review rounds completed in one session.** Round 1 (3-voice — claude-review + user-dispatched Codex + Hermes) surfaced 4 substantive findings; all 4 accepted in fix-up `55d8510`. Round 2 (single-voice claude) surfaced 7 findings; 1 cosmetic accepted in fix-up `f66b992`. Round 3 (single-voice claude) surfaced 6 findings; all default-rejected per kickoff round-cap rule.
- **Pre-push code-reviewer agent on round-1 fix-up returned 0 findings** with verdict "Approve as-is." Independent Opus-model agent ran focused review of the fix-up diff (F1 gate placement / F2 completeness / F3 internal consistency / F7 type narrowing / hidden regressions); confirmed no P1/P2/P3 issues.
- **Negative-path probes for accepted fixes** all reproduced pre-fix bug and passed post-fix:
  - F1: `--apply` against worksheet with removed verdict line → pre-fix `--apply` exit=0 + wrote; post-fix `--apply` exit=1 + md5 unchanged.
  - F2: `LC_ALL=C PYTHONUTF8=0 python3 ... --verify-only` → pre-fix `UnicodeDecodeError`; post-fix exit=0.
  - F3: §9.1.1 mutated to `filter_ui_tier: \`oops\`` → pre-fix `--verify-only` exit=0; post-fix exit=1 with `INVARIANT FAILURE: 9.1.1: unknown filter_ui_tier 'oops'` + `tier 'top': expected 12 entries, found 11`.
- **PR #492 ship cycle:** user authorized squash-merge after my recommendation that round-cap rule applied at round 3 (no critical bugs, 4 of 6 findings repeats from prior rounds, 2 net-new were cosmetic ergonomics + bot misread of an AND-condition). Squash-merged 2026-05-12 as `e5f4257` via `gh pr merge --squash --delete-branch`. Local `main` fast-forwarded.
- **Closeout-backfill direct-to-main per Sessions 67 + 68/PR #485 + 69 + 71 + 73 + 74 + 75 precedent — 8th occurrence; user-authorized this session.** Dashboard row added, Last updated banner refreshed, Next session contract updated for Session 77 (TBD), PR closeout checklist marks, Current State paragraph extended with Session 76 paragraph, What's done / What's NOT done / For next session bullets updated, this Session 76 log entry added.

**Decisions made this session:**

- **Path (a) over (b) for §16 regeneration.** User-chosen at session start. Tradeoffs surfaced: (a) higher up-front cost (~441 lines of Python + tests) but reusable for Stage 2 worksheets vs (b) lower up-front cost but one-off throwaway. Session 59 Decision 1 (2-level-flexible hierarchy) implies multiple Stage 1 / Stage 2 worksheets will follow similar labeled-line per-value entry shape; reuse benefit is real, not hypothetical.
- **Merge target inline encoding in `verdict` cell** preserved from Session 74 §16 transcription (`merge → <target_canonical_key>`). Structured `merge_into` stays in per-value entries per §7 parsing convention; §16 is presentation surface, not parser-reversible.
- **F3 verifier hardening: `<to_fill>` is in the verdict whitelist but excluded from the verdict-set distribution check.** Reasoning: the existing `<to_fill>` count check is the sole owner of that failure mode (separate, clearer error message). Adding `<to_fill>` to the unknown-verdict check would produce noisier output for the same failure scenario.
- **F3 verifier hardening: `split` / `drop` deliberately excluded from verdict whitelist.** Per §3 they are theoretical verdicts but not present in the current frozen worksheet (after curriculum-team fill); if a future Stage 2 worksheet uses them, the parser should fail loud and prompt explicit thinking about how `split`/`drop` semantics map to canonical-vocab consumers, not silently pass them through.
- **F1 `--apply` gate uses hard-fail, no `--force` escape hatch.** Reasoning: this is a mechanical-regeneration tool. If a regeneration shouldn't pass `verify_invariants`, the source per-value entries need fixing, not a bypass. A `--force` flag is the kind of bypass that gets used reflexively and undermines the gate's purpose.
- **Round-cap rule applied at round 3.** Per kickoff prompt: "Round-cap after 2 bot-review rounds. If a 3rd round appears, fix only critical bugs, document the rest, and ship." Round 3 had 0 P1 findings; bot's own verdict was "Approve with minor nits, doesn't block merge"; 4 of 6 findings were repeats from prior rounds; 2 net-new were either bot misread (R3-4 AND-condition) or cosmetic ergonomics (R3-3 FileNotFoundError friendliness). All 6 default-rejected with rebuttals; rebuttal comment posted to PR.

**Process notes / observations:**

- **Total ship cycle: 1 Claude session.** Orient + read parser convention + build script + verify + ship + 3 bot-review rounds + 2 fix-up cycles + merge + closeout-backfill. The fix-up-commit-per-round pattern combined with `gh pr merge --squash --delete-branch` was efficient.
- **5/14 findings across 3 rounds accepted; 9 rejected with rebuttal.** Bot acceptance rate ~36% — consistent with the round-cap rule's expectation that round 2 + 3 yield diminishing returns. Round 1 acceptance rate 4/7 = 57%; round 2 was 1/7 = 14%; round 3 was 0/6 = 0%. The acceptance trajectory follows the expected pattern.
- **Two-voice convergence as P1-equivalent signal worked again.** F2 UTF-8 was a 3-voice convergence (claude + Codex + Hermes); accepted without further debate. Single-voice rounds 2 + 3 had no convergence amplifier, which aligns with the kickoff prompt's "absence of convergence correlates with absence of P1" framing.
- **Codex + Hermes round-2/3 not dispatched.** User dispatched these manually in round 1 only. With round 1's 3-voice picture establishing the substantive scope, single-voice rounds 2 + 3 were sufficient to confirm no P1 emergence. If a future PR has higher stakes (e.g., a real schema migration), the 3-voice pattern would re-apply.
- **R3-4 was a bot misread.** The bot interpreted `if "<" in rest and "<cluster>" in line` as if the `"<" in rest` term alone could match a real entry; the AND-condition with the literal `<cluster>` anchor was missed. Worth recording: bots occasionally misread conditional logic; rebuttals that quote the exact source text are clearer than rebuttals that paraphrase.
- **Default rejecting Security Audit pre-existing fail across all 3 rounds** followed the established memory pattern (`@lhci/cli` chain hygiene-follow-up; same posture as PR #491 and earlier docs/tooling Stage 1 PRs).

**What was NOT done this session:**

- Codex + Hermes round-2/3 dispatch (round 1 only; user-discretion).
- Stage 2 audit signal register updates — parser-driven §16 regeneration is a mechanical operation on the now-frozen per-value entries, surfaced no new corpus signals.
- Foundation-phase code track engagement — PR 3b / 5 / 6 all still gated on Stage 1 / Stage 2 outputs.
- Reactions to the 2 user-side worktree artifacts (Q&A kickoff doc + heritage-worksheet-form/ directory) — user-owned, not in PR #492 scope.

**Next:** Session 77 = TBD by user direction. Stage 1 has no inherent next step now that content + scaffolding + structural §16 regeneration are all shipped. See Next session contract block at top of doc for the 3 plausible pivots and the literal "decide pivot direction with user at session start" guidance.

---

### Session 75 — 2026-05-12 — Curriculum-team fill integration + PR #491 ship cycle

**Branch:** `docs/stage1-heritage-curriculum-team-fill` (off `main` at `87d024a` — the post-Session-74 commit including the curriculum-team handoff kickoff doc). PR #491 squash-merged 2026-05-12 as `d8af791`; origin auto-deleted via `--delete-branch`.

**Done:**

- Verified curriculum-team-returned worksheet (delivered via Downloads filesystem) against the substantively-complete-pre-handoff version on main. Methodology / scaffold §1-§8 byte-identical. 88 per-value entries given verdicts (69 keep / 17 merge / 2 new in initial fill). §9.1 framing decisions D1-D5 answered: D1 "Indigenous and Diaspora" cluster root; D2 `African American` canonical with AA-diaspora retired/merged; D3 `Indigenous` canonical with `Native American` + `Indigenous/Native American` + `Indigenous Peoples` merged as aliases; D4 tribal nations (Lenape, Haudenosaunee) parented directly under `indigenous`; D5 v3-corpus-absent values kept as `internal`-tier placeholders for future tagging.
- Surfaced 2 alignment issues to user pre-copy-over: (1) D5 written as "Drop them all" but actual verdicts on all 10 zero-corpus entries (§9.1.10-13 + §12.18 Dominican + §14.10/11 French/Polish + §15.7-9 Lebanese/Syrian/Jordanian) were `keep` + `internal` — contradiction; (2) §9.1 merge entries §9.1.3 / §9.1.5 / §9.1.6 / §9.1.7 had `parent: <to_fill>`. User clarified: D5 intent matches the verdicts ("keep them all but internal"); §9.1 merge entries should parent under `indigenous-and-diaspora`. Applied both reconciliation edits to the Downloads file before copy-over.
- Copied filled file to repo. Branched off main as `docs/stage1-heritage-curriculum-team-fill`. Pre-push Opus code-reviewer agent verified all 88 verdicts, 17 `merge_into` pointers, parent-chain references, drift-entry §7 compliance, D1-D5 alignment, and §1-§8 scaffold preservation — returned "Ship as-is" with 0 P1-P4 findings.
- Opened PR #491. 3 claude-review rounds + 2 fix-up cycles:
  - **Round-1** surfaced P3 (14 keep-vs-new misclassifications per §3 spec — north-african / east-african / central-american / south-american / southern-united-states / honduran / cuban / kenyan / egyptian / moroccan / irish / yemeni / persian / sri-lankan) + P4 (Cajun/Creole parent divergence — inline-comment fix) + P5 (Haudenosaunee tier — invalid, bot misread Lenape count as 3 vs actual 7). Accepted P3 + P4 in fix-up `ce74ea6`; default-rejected P5 + Security Audit with rebuttal.
  - **Round-2** surfaced 4 more keep-vs-new misses (peruvian / brazilian / ecuadorian / guyanese — closing the round-1 list gap; verified via local audit against Appendix A) + P3 D3-wording (default-reject: framing-block prose not parsed per §7) + P5 AA-diaspora-parent oddity (bot itself flagged "no action required"). Accepted P2 in fix-up `7c092da`; default-rejected P3 + P5 + Security Audit.
  - **Round-3** returned "Ship-ready" with 5 observations all default-rejected per round-cap policy (Stage 2 territory / already-handled / intentional).
- User authorized squash-merge. PR #491 squash-merged 2026-05-12 as `d8af791` via `gh pr merge --squash --delete-branch`. Local `main` fast-forwarded to merge commit.
- Closeout-backfill direct-to-main per Sessions 67 + 68/PR #485 + 69 + 71 + 73 + 74 precedent (7th occurrence; user-authorized this session): dashboard row added, Last updated banner refreshed, Next session contract updated for Session 76, PR closeout checklist marks, Current State paragraph extended, What's done / What's NOT done / For next session bullets updated, this Session 75 log entry appended.

**Decisions made this session:**

- **Reconciliation edits before copy-over preserve the team's intent over their literal wording.** D5's "Drop them all" + verdicts of `keep` on all 10 zero-corpus entries was a self-contradiction; user clarified intent matched the verdicts. Rewrote D5 wording rather than flip verdicts. Similarly, 4 §9.1 merge entries with `parent: <to_fill>` were filled per user direction to `indigenous-and-diaspora` (cluster root), matching the drift-entry-parent convention rather than the merge_into-target.
- **Round-1 bot's list of 14 keep-vs-new misclassifications was incomplete.** Round-2 surfaced 4 more (peruvian / brazilian / ecuadorian / guyanese) that round-1 missed. Lesson: when a bot lists "N findings of category X", treat it as a starting set, not the complete set. Local audit against Appendix A in round-2 confirmed the round-2 list was complete (2 false-positives surfaced by my own script — three-sisters-traditions and black-culinary-history — turned out to be v3-baseline values flagged in framing-block question 5).
- **D3 framing-block prose default-reject (bot round-2 P3).** Per §7 parsing convention, parsers read per-value entries only; D3 is framing-block prose for human readers. "Merge X + Y as aliases" is acceptable shorthand even though `merge` and `aliases` are distinct operations per §3 — the per-value entries on §9.1.5 / §9.1.6 say `verdict: merge` correctly. No structural risk.
- **Cajun/Creole parent inline-comment vs flipping to `african-american`.** Bot's round-1 P4 corroborated my pre-push P5 awareness note. Chose the inline `<!-- ... -->` annotation rather than re-parenting per user-implicit consent (the team's parent choice "indigenous-and-diaspora direct" reflects substantive judgment that Louisiana foodways aren't reducible to AA identity; annotation documents the deliberate divergence for future readers).

**Process notes / observations:**

- Total ship cycle for curriculum-team fill: 1 Claude session (orient + verify + reconcile + copy + branch + 3 commits + 3 review rounds + 2 triage cycles + merge + closeout-backfill). The fix-up-commit-per-round pattern combined with `gh pr merge --squash --delete-branch` was efficient.
- 6/9 bot findings across 3 rounds were accepted; 3 rejected with rebuttal. Notable mistake pattern: the round-1 bot's enumerated lists weren't exhaustive — both the keep-vs-new list (missed 4 entries) and the round-2 list ended up being the canonical set. Lesson reinforced: do local audits against the spec, not just trust bot lists verbatim (memory `feedback_verbatim_identifiers_in_probes.md` applies here in spirit).
- Default-rejection of round-3 bot findings followed the kickoff round-cap rule (round-3 = critical-bugs-only). All 5 round-3 observations were genuinely non-critical: Stage 2 territory, already-handled, or intentional v3-by-design.
- Security Audit pre-existing fail (`tmp` / `ip-address` / `postcss` / `basic-ftp` / `@lhci/cli` chain) default-rejected across all 3 rounds — same memory-tracked hygiene follow-up that's affected every recent docs-only Stage 1 PR. Out of scope per Stage 1 branch policy.
- The 8-row dashboard now has a 9th row (Curriculum-team fill integration). Future Stage 1 rows (if any — §16 regeneration would qualify) follow this same pattern.

**What was NOT done this session:**

- §16 end-summary verdict-cell regeneration. The §16 table still carries `<to_fill>` verdict cells per the "Do not hand-edit — mechanically regenerated" rule. Session 76 candidate scope.
- Per-cluster decision summary prose blocks (§5.3) — always TBD / curriculum-team-optional.
- Audit signal register additions — PR #491 was a verdict integration, not a corpus read; no new signals.
- Foundation-phase code track engagement — PR 3b / 5 / 6 all still gated on Stage 1 / Stage 2 outputs.

**Next:** Session 76 = TBD by user direction. Three options surfaced in Next session contract: (a) §16 regeneration; (b) pause; (c) pivot to foundation-phase code track if PR 3b / 5 / 6 unblock.
...
### Session 73 — 2026-05-11 — PR #489 review cycle (round-1 fix-ups + round-2 ship-ready + squash-merge + direct-to-main backfill)

**Branch:** `docs/stage1-heritage-cross-cluster-and-end-summary` (continued from Session 72; squash-merged 2026-05-11 23:34 UTC as `3ea3ecb`).

**Done:**

- Triaged 2 round-1 bot reviewers (Claude auto-bot + Codex manual) on PR #489 — findings collected from all 4 PR surfaces (issue comments, formal reviews [empty], inline comments [empty], checks). Both bots converged 2-voice on 2 P2 findings; Codex surfaced a third single-voice P2.
- **Round-1 acceptance (3 P2 findings, all accepted):**
  - **P2 #1 (2-voice convergence — pre-push outcome staleness):** Both bots flagged "Pre-push code-reviewer agent: TBD at session 72 end" persisting in foundation status doc line 156 + Stage 1 status doc lines 86 + 198 despite commit `84201aa` shipping 3 fix-ups. Updated all 3 spots with the actual 3-finding outcome (P2-1 + P2-2 + P3-1 from `84201aa`) + "Thirteenth consecutive Stage 1 pre-push agent catch" framing matching Session 70's tenth-catch precedent.
  - **P2 #2 (2-voice convergence — placeholder sweep):** Both bots flagged `PR # TBD-end-of-session` / `TBD-by-session-72-end` / `Session 72 PR # TBD` placeholders unresolved now that PR is #489. Swept all 9 occurrences across foundation status doc (lines 9, 14, 45, 145, 156) + Stage 1 status doc (lines 21, 31, 57, 86, 198) to `PR #489 OPEN` / `PR #489` / `#489`.
  - **P2 #3 (Codex single-voice; investigated + accepted):** Codex flagged worksheet §9.1 alias arrays at lines 379/407/420 containing embedded prose like `[African American diaspora (2 corpus lessons; both problem rows — see §9.1.3 + audit signals X-04 + X-05)]` instead of the spec's `["alias_1", "alias_2", ...]` shape (spec line 150). Verified against 39+ prior alias entries in the worksheet (all `[]` or quoted-string arrays) + parsing convention §7. Accepted as real spec-conformance drift. Fixed to: §9.1.2 → `["African American diaspora"]`, §9.1.3 → `["African American"]` (with one-line `<!-- ... -->` alternative-direction note per spec line 174 inline-review-comment allowance), §9.1.4 → `["Native American", "Indigenous/Native American", "Indigenous Peoples"]`. Count + rationale info preserved in adjacent Notes paragraphs.
- **Round-1 default-rejects (single-voice low-priority):** Claude P3 catch-streak count internal-consistency (agent verified the count math is correct — informational only, no fix needed).
- Round-1 fix-up commit `3e45b5c` (5 edits foundation status doc + 6 edits Stage 1 status doc + 3 edits worksheet).
- **Round-1 pre-push code-reviewer agent (Opus, feature-dev:code-reviewer) on the fix-up diff returned 0 findings.** **Thirteenth Stage 1 catch-streak watch.** The pre-push agent ran a thorough cross-surface consistency check (catch-streak arithmetic 6+1+2+1+2+1=13, alias-array spec compliance, placeholder coverage, lurking stale strings) and gave the fix-up diff a clean verdict.
- **Round-2 auto-Claude verdict: ship-ready** with 1 P2 + 2 P3:
  - **P2 (PR description stale) — accepted via PR body edit, not new commit.** PR body didn't mention commit `3e45b5c`. Updated via `gh pr edit --body-file` with new "Round-1 fix-ups" + "Round-2 fix-ups" sections (the latter documenting the 2 P3 default-rejects in-line with rationale). No new commit triggers another auto-review round.
  - **P3 (~10 multi-tagged overlaps marker) — default-rejected with rationale.** The `~` reflects the row-vs-occurrence-count ambiguity (rows tagged in 3+ canonicals contribute >1 overlap each); removing it would falsely assert "10 distinct multi-tagged rows" when the arithmetic is "10 tag-appearance overlaps which could be ≤10 rows." Reviewer's own spot-check used the (a) interpretation but `~` preserves the broader semantic. Rationale captured in PR body.
  - **P3 (parent dependency note) — reviewer self-rejected** as "consistent with existing convention" (§12.18 Dominican precedent).
- Codex round-2 not dispatched (user-authorized auto-Claude-only path per Session 69 + Session 71 precedent: "minimal value given convergence + round-cap rule").
- CI on `3e45b5c`: all green except known baseline `Security Audit` `@lhci/cli` flake.
- **PR #489 squash-merged 2026-05-11 23:34 UTC as `3ea3ecb`** via `gh pr merge --squash --delete-branch`. Origin branch auto-deleted; local branch auto-switched to main + fast-forwarded.
- Session 73 closeout backfill direct-to-main per Sessions 67 + 68/PR #485 + 69 + 71 precedent (5th occurrence — well past 3rd-occurrence promotion threshold; user-authorized).

**Decisions made this session:**

- **PR body update via `gh pr edit` as alternative to new commit for round-2 PR-description P2.** Round-2 claude-review's "PR description stale" P2 had two paths: update PR body now OR defer to squash-merge time. PR-body-edit (no new commit) avoided triggering Round-3 auto-review while still addressing the finding — strictly better than a new docs-only commit. Worth flagging for future round-2 sessions: PR-description-staleness findings can be addressed via `gh pr edit` without re-entering the bot-review cycle.
- **`~` approximation marker default-reject vindicated by tag-appearance vs multi-tagged-row distinction.** The "removes ~10 multi-tagged overlaps" wording captures a real semantic ambiguity (10 tag-appearance overlaps, but the multi-tagged-row count is ≤10 because rows tagged in 3+ canonicals contribute >1 overlap each). Reviewer's spot-check used the (a) interpretation; the `~` preserves the broader semantic. Default-rejected with rationale captured in PR body for future-reader auditability.

**Process notes / observations:**

- **Round-2 single-fix-up via PR body edit avoids Round-3 trigger.** Per Session 71 observation, Round-3 should be reserved for critical bugs. PR-description-staleness P2 was addressed via `gh pr edit` (no commit, no auto-review re-trigger), preserving the round-cap discipline. Worth flagging as a candidate `feedback_*.md` rule if it recurs: round-2 PR-description-staleness findings → `gh pr edit` not new commit.
- **2-voice convergence on round-1 P2s was clean.** Both Claude auto-bot and Codex manual found the same 2 P2 findings (TBD pre-push outcome + placeholder sweep) independently, with very similar suggested wording. Strong "bot voice convergence as P1 signal" pattern.
- **Codex's single-voice P2 alias-prose finding earned accept.** Despite being single-voice, Codex's spec-cite (line 150) + parsing convention (§7) + the 39+ prior alias entries' pattern made the finding's basis verifiable independent of bot count. Demonstrates the "investigate single-voice findings against spec/convention before triaging" pattern from `feedback_bot_review_investigation.md` (memory-tracked).
- **Pre-push agent on fix-up diff returned 0 findings.** Sessions 67 (PR #484 fix-up) + Session 71 (parts) had similar clean returns. Pattern: when fix-ups are direct mechanical applications of accepted bot findings (no new content / no new arithmetic / no new audit signals), the pre-push agent usually returns clean. Pre-push agent's value in those cases is structural verification (catch-streak math, placeholder coverage, cross-surface consistency) rather than new defect discovery.
- **Session 73 = tighter session boundary than Session 71.** Session 71 ran 100% review-cycle + closeout. Session 73 was even tighter: round-1 fix-ups (commit) + round-2 (PR body edit, no commit) + squash-merge + closeout. No round-3, no pre-push agent re-dispatch on round-2 PR-body-edit. Clean 4-stage pattern: triage → fix-up → ship-ready → merge.
- **All 5 regional clusters + cross-cluster §9.1 shipped.** Session 73's merge of PR #489 lands the worksheet's substantive content. Remaining Stage 1 work: §16 end-summary canonical-vocab table (Session 74) + curriculum-team handoff (post-Session-74 milestone).

**For next session (Stage 1 Session 74 = §16 end-summary canonical-vocab table):**

See `## Next session contract` block at top of doc for the authoritative summary. Branch off `main` at the Session 73 closeout-backfill commit. Primary objective = populate §16 end-summary canonical-vocab table mechanically from filled §11-§15 + §9.1 per-value entries (~88 rows). After Session 74 ships, the next major Stage 1 milestone is curriculum-team handoff (worksheet substantively complete, awaits external review). Foundation-phase code track still has no unblocked next PR.
