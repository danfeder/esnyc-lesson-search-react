...
- **NEXT:** dispatch PR-2 Task 2.1 (3-wave `Promise.all` rewrite + thread the R2-1 reviews-error block into Wave A + the 2 trivia folds) to a fresh-context executor; supervisor-verify (page net 16/16 green + `npm run check` + diff spot-check); GATE-3 (reviewer agent + Codex); the **load-bearing manual smoke** (the mock ignores query args → can't catch a dependency-violating reorder); then push + `gh pr create`.
