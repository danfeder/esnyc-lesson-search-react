...
- **⚠️ F1 (PR-1a round-1 bot, Medium) — `ReviewDocPanel` Google-Doc-embed `onError` uses `logger.error` (→ Sentry `captureException` at ERROR severity) for a GRACEFULLY-RECOVERED state** (component immediately `fallbackToText`s → reviewer sees the extracted text, no visible error). Produces Sentry noise on every CSP-block / firewall-timeout. **Should be `logger.warn`.** PRE-EXISTING (carried verbatim from the original `ReviewDetail.tsx`); DEFERRED out of PR-1a to keep it byte-identical (GATE-4 Codex = AGREE-DEFER). **Explicit follow-up (do NOT rely on proximity — Codex caveat):** fix in PR-1b when `<ReviewDocPanel>`/the decision column area is next touched, OR a tiny dedicated cleanup commit. One-line change at `src/components/Review/ReviewDocPanel.tsx` `onError` handler.
- **F5 (PR-1a round-1 bot, Low, cosmetic) — alias-then-rebind naming in `ReviewDetail.tsx`:** `validateRequiredFields as computeRequiredFieldErrors` import + a local `const validateRequiredFields = useCallback(...)` wrapper reusing the original name. Readable-but-confusing; suggested cleaner scheme = import as `validateRequiredFields`, name the memoized wrapper distinctly (e.g. `getValidationErrors`). DEFERRED to PR-1b (it reworks the LEFT/RIGHT columns + this validation wiring); pure style, no behavior. Same applies if the `computeFieldProgress`/`fieldProgress` pair reads confusingly.
