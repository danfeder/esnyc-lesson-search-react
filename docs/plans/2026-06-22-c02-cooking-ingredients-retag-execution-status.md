...
**Last updated:** 2026-06-23 by Session 3 (P1.3 + P1.4 done + supervisor-verified; next P1.5)
...
**Current task:** **P1.1 + P1.2 + P1.3 + P1.4 COMPLETE + supervisor-verified.** Next: **P1.5** — 3-layer answer-key sample strata in `sample-answer-key.ts` (`stratumKey` ~L244). Add (1) **hard-case quotas** keyed off current-tag values (vague cooking tags `Basic Skills`/`Cooking Techniques`, the `Herbs & Aromatics` 48-catch-all, orphan foods); (2) a **deterministic seeded set-cover** guaranteeing every group + every specific appears ≥2× — **target = expected post-retag membership** (seed from the alias-floor predictions / a pre-scan, NOT current tags, since added specifics have near-zero current carriers); (3) a **clean-core slice** retaining `activity_type×body-length-quartile` balance. **Size 70** (≈25 clean-core / 25 coverage / 20 hard-case). TDD: the key covers every group + specific ≥2×; layer sizes as expected; deterministic under a fixed seed. Then **P1.6** (4-gate scoring + rules baseline) closes P1. **P1.5 is the algorithmically heaviest P1 task** (set-cover + post-retag-membership seeding) — author the executor prompt + verify with fresh care.
...
**Branch:** `feat/c02-harness` (off `chore/c02-scaffold` @ `8834ec6`). Landed: P1.1 (`83da03f` + `e5fed38`), lock-correction docs (`f619a66`), P1.2 (`18b3ebc`), P1.3 (`272ee1a` + fix-up `3ddbdfc`), checkpoint `549e096`, **P1.4 (`05a3b437`)**. P1.5–P1.6 next. The P1 PR (→ main) bundles the 2 planning commits + lock-correction docs + all P1 implementation commits.
