...
**Last updated:** 2026-06-22 by Session 3 — **PR A MERGED (#532, squash `5b5226a`).** PR B (archival) executed locally on `chore/wave3-archival`: B.1 mechanism locked (`docs/plans/archive/` subdir), B.2 classified (**56 archive / 18 keep**, Codex-cross-examined), B.3 manifest written + B.4 `git mv` done. Pre-push review next; not yet pushed/opened.

### Current State
**Active PR:** **PR B — docs archival** on `chore/wave3-archival` (off `main` @ `5b5226a`). PR A (#532) **MERGED** 2026-06-22 (squash `5b5226a`). B.1–B.4 done locally (not yet pushed/opened): `docs/plans/archive/` subdir + `ARCHIVAL_MANIFEST.md` + 56-file `git mv` + live-pointer/§3/tracker bookkeeping folded in.
**Current task:** **PR B — pre-push code-reviewer → `npm run check` → push → PR → four-surface triage.** No DB/edge gates (docs-only).
...
### In flight
- **PR B — docs archival** — executed locally on `chore/wave3-archival`; pre-push review pending. **56 docs `git mv`'d** into `docs/plans/archive/` + `ARCHIVAL_MANIFEST.md` created; **18 kept** in root (4 live campaign · 4 functional script inputs · 2 live-command-referenced · 2 deferred-future-work inputs · 6 live-source-comment Tier-4b). Classification **Codex-cross-examined** (zero missed refs on the 56; Tier-4 split verified; Option B recommended + user-chosen). Live navigational pointer (roadmap "see …theme-b…") repointed to `archive/`; ~150 archive→archive intra-links left stale by design (manifest is the map). `npm run check` re-verify + pre-push code-reviewer next, then push → PR → four-surface triage.
...
- **B.3/B.4** — created `docs/plans/archive/` + `ARCHIVAL_MANIFEST.md` (grouped by initiative + a "kept in place / why" section documenting the 18) + `git mv` 56 files (56 moved / 0 failed; verified 18 KEEP remain top-level). Folded the PR-A-merged status into the master tracker + roadmap live-pointer + this §3.
- **Next:** `npm run check` → pre-push code-reviewer → push → `gh pr create` → four-surface triage.
