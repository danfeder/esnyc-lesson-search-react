...
- **Next:** re-run the TEST-dispatch rehearsal on the amended branch (now expected WARN/PASS — lenient path, since a manual dispatch of unchanged `send-email` is a content no-op) to confirm the API integration + lenient branch end-to-end → open PR → four-surface triage → **user's merge go** (PROD-pipeline workflow; never merge without it). Branch already pushed; gates already run pre-push.
