...
- Teeth-tested the verify decision logic locally (bash): real-deploy / first-deploy / degraded-pre PASS; **silent no-op (pre==post) → FAIL exit 1**, regression / non-ACTIVE / non-numeric → FAIL. YAML validated (ruby parse).
...
- **Next:** TEST-dispatch rehearsal (`gh workflow run deploy-edge-functions.yml --ref chore/wave3-c33-edge-deploy-verify -f environment=test -f function=<fn>`) → pre-push gates (code-reviewer + Codex) → push → PR → four-surface triage → **user's merge go** (PROD-pipeline workflow; never merge without it).
