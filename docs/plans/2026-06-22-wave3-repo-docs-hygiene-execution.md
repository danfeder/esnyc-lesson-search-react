...
**Last updated:** 2026-06-22 by Session 5 (C33 branch) — **Wave-3 docs core COMPLETE: PR A #532 `5b5226a` · PR B #533 `7367f20` · PR C #534 `063bd4b`, all MERGED.** The re-homed tail is now tracked in the master tracker + `[[project_deferred_work_campaign]]`: **C169+C31 Dependabot batch DONE** (Phase 1 + Phase 2 merged; deferred majors #460 eslint-10 / #451 TS-6.0 left open), **`Security Audit` CI check FIXED** (#535 `4ee9cea`), **C33 (edge-deploy post-deploy verify + serialize matrix) ✅ MERGED `a8efac9` (#537)**, **C40 (memory archive-split) out-of-repo (no PR), done**. **Wave 3 = ✅ DONE → Wave 4 SCAFFOLDED 2026-06-22** (`2026-06-22-wave4-data-corpus-cleanup-*`; design-lock mode, Session 1 = design-lock next).

### Current State
**Docs core (PR A/B/C) COMPLETE** — all three merged to `main`; this execution doc's core scope is closed. The remaining Wave-3 tail (C33 edge-deploy verify, C40 memory split) was re-homed off the docs track and is tracked in the master tracker + `[[project_deferred_work_campaign]]`.
**Done (re-homed):** **C33 — deploy-edge-functions post-deploy verify + serialize the matrix — ✅ MERGED `a8efac9` (#537) 2026-06-22, branch deleted** (was `chore/wave3-c33-edge-deploy-verify`, off `main` @ `4ee9cea`). Adds a per-function pre/post version-increment check to `deploy-test` + `deploy-prod` and drops `max-parallel: 4` → `1`. Touches the PROD edge-deploy pipeline → standing edge gates apply (TEST-dispatch rehearsal + 3-signal verify discipline + never-merge-without-user-go).
**Last commit on main:** `4ee9cea` (#535 — Security Audit fix; landed after PR C `063bd4b`).
**Baseline:** `npm run check` green (workflow-YAML change only; no TS/lint surface).
...
- ✅ **PR B — docs archival (#533)** — MERGED 2026-06-22, squash `7367f20`. **56 docs `git mv`'d** into `docs/plans/archive/` + `ARCHIVAL_MANIFEST.md`; **18 kept** in root (4 live campaign · 4 functional script inputs · 2 live-command-referenced · 2 deferred-future-work inputs · 6 live-source-comment Tier-4b). Three independent gates agreed (pre-push Claude **SHIP** · Codex **AGREE-SHIP** · CI `claude-review` **APPROVE**); Security-Audit = pre-existing npm-audit noise (rejected). `[user-verdict]` Tier-4 = Option B.
- ✅ **PR C — migration-doc 4→2 consolidation (#534)** — MERGED 2026-06-22, squash `063bd4b`. Collapsed the 4 overlapping migration surfaces → 2 canonical (root `CLAUDE.md` quick-ref + `supabase/migrations/CLAUDE.md` detailed guide, which absorbed the unique bits); `database-migrations` skill left **untouched** (NON-RETIRABLE decision tree); `docs/MIGRATION_WORKFLOW.md` → ~16-line redirect stub (`[user-verdict]` = stub, not delete); `new-migration.md` → thin shell. Three independent gates agreed (Claude **SHIP** · Codex **AGREE-SHIP** · CI `claude-review` **APPROVE**). **→ Wave-3 docs core (C60+C39) COMPLETE.**

### Done (re-homed tail)
- **C33 (re-homed) — ✅ MERGED `a8efac9` (#537) 2026-06-22, branch deleted** — deploy-edge-functions post-deploy verify + serialize the matrix. Added a pre-deploy version capture (`id: pre`) + a post-deploy version-increment assertion (fail loudly on the silent no-op — the bug that recurred on `complete-review` ×2 / `password-reset` ×1) to BOTH `deploy-test` and `deploy-prod`; serializes `max-parallel: 4` → `1` (`fail-fast: false` kept). Verify-step decision logic teeth-tested locally (silent no-op → exit 1). **Next:** TEST-dispatch rehearsal to exercise the new step on a real deploy → pre-push gates (code-reviewer + Codex) → push → PR → four-surface triage → user's merge go. C40 (memory archive-split) is out-of-repo (no PR), done in the same session. Detail in the master tracker + `[[project_deferred_work_campaign]]`.
...
#### Session 4 — 2026-06-22 — PR C "migration-doc 4→2 consolidation" execution
- Session-start ritual: git diverged from §3 as flagged in memory — **PR B (#533) had MERGED** (squash `7367f20` on `main`); §3 said "OPEN, awaiting merge." Trusted git. On `main`, clean except the 6 intentionally-untracked items (left alone). `npm run check` green baseline. Read all 4 migration surfaces + the `database-migrations` skill.
- **C.1 lock:** consolidation target = decision-#8 default (root `CLAUDE.md` quick-ref + `supabase/migrations/CLAUDE.md` canonical; skill = NON-RETIRABLE decision-tree source; `new-migration.md` = thin shell). Evidence for the `[user-verdict]`: `git grep` found the **only live inbound ref** to `docs/MIGRATION_WORKFLOW.md` is `CLAUDE.md:83` (edited anyway); all others are in already-archived/superseded docs (`TECH_DEBT_AUDIT` SUPERSEDED-banner'd, `docs/archive/*`). Recommended redirect-stub; **user chose redirect-stub** via AskUserQuestion.
- Confirmed PR B's reference scan held: `supabase/migrations/CLAUDE.md:225` → `2024-12-04-periodic-baselining-design.md` is **not dangling** (PR B correctly KEPT that doc at root as a live `/baseline-db` input; manifest documents it).
- Branched `chore/wave3-migration-docs` off `main`.
- **C.2 consolidation** (5 files): (1) canonical `supabase/migrations/CLAUDE.md` += pipeline-flow visual + Local Development Setup (Studio URL / start-stop) + **GitHub Workflows table** (the highest-value unique bit — names `e2e.yml` / `migrate-production.yml` / `reset-test-db.yml`); (2) `docs/MIGRATION_WORKFLOW.md` → ~16-line redirect stub (→ canonical + skill + root quick-ref + `/new-migration`); (3) root `CLAUDE.md` — line-28 read-first pointer now names the skill, See-Also drops the now-stub MIGRATION_WORKFLOW bullet and adds the skill (decision tree); (4) `.claude/commands/new-migration.md` → thin shell pointing at skill+canonical, **gaining** the same-day-timestamp ASCII gotcha + the TEST-DB-verify-before-merge mandate it was missing; (5) skill **untouched**.
- Bookkeeping fold (trust-git-then-fix-§3): master tracker lines 7+15 (PR B MERGED · PR C in flight) + this §3. Roadmap live-pointer already correct on main (no change).
- **Next:** C.3 verify → C.4 pre-push gates (code-reviewer + Codex GATE 3) → push → PR → four-surface triage.
- **PR C MERGED 2026-06-22 (#534, squash `063bd4b`)** — three gates agreed (Claude SHIP · Codex AGREE-SHIP · claude-review APPROVE). → Wave-3 docs core (C60+C39) COMPLETE.

#### Session 5 — 2026-06-22 — C33 (re-homed) edge-deploy verify + matrix serialize · branch `chore/wave3-c33-edge-deploy-verify`
- Session-start ritual: clean `main` @ `4ee9cea` (PR C `063bd4b` + Security Audit fix #535 both landed after Session 4's edits — trusted git; folded the "PR C merged → docs core complete / C169 Phase 2 done / Security Audit fixed" corrections into this branch's first commit). Only the expected untracked `*-kickoff.md` + `heritage-worksheet-form/` present (left alone).
- Confirmed the edge-function Management API response shape via MCP `get_edge_function` (`version` int + `status` + `ezbr_sha256`); chose **version-increment** as the automated equivalent of the manual 3-signal post-deploy check — signal #1, the most reliable, and the only one needing no TEST-vs-PROD cross-ref or local source reproduction in CI.
- Edited `deploy-edge-functions.yml`: added `Capture pre-deploy version` (`id: pre`, Management API GET) + `Verify deploy advanced the version` (poll + assert post > pre, ACTIVE, numeric) to both `deploy-test` + `deploy-prod`; `max-parallel: 4` → `1`.
- Pre-push gates (Claude `feature-dev:code-reviewer` + Codex cross-exam, run in parallel): folded 2 real fixes — (a) a 200-but-non-numeric pre-version is now **fatal** (was a silent warn-pass that voided the teeth), (b) guarded the trailing poll `sleep`; documented Codex's "version is a project-level counter" concurrency caveat in-step (rare residual; manual 3-signal stays the backstop). Token handling / output wiring / serialization confirmed sound by both.
- **TEST-dispatch rehearsal (`send-email`, run `27969761550`) surfaced the key empirical fact: Supabase only bumps `version`/`ezbr_sha256` when the bundle SOURCE changes — an identical redeploy is a content no-op (version stayed 13→13).** So a blanket strict "must advance" check false-positives on identical redeploys (incl. the rehearsal method, re-runs, `_shared` fanout to a non-importer). **User decision (AskUserQuestion): version-advance, _push-strict_** — `detect-changes` now emits a `strict` output (true ONLY for automated push/PR deploys of specific changed functions, where the source must change); the verify step FAILS on a frozen version when strict, WARNS otherwise. All 3 historical no-ops were push-path own-changed functions → still caught.
- Teeth-tested locally (strict + lenient): strict+frozen → FAIL (the real bug); lenient+frozen → WARN/PASS (fixes the rehearsal false-positive); first-deploy / advanced → PASS; non-ACTIVE / non-numeric pre/post → FAIL. YAML validated (ruby); `npm run check` green.
- Opened **PR #537** + ran the gate rounds. **claude-review APPROVE** — folded 2 nits (poll breaks only when version AND status present; commented the unreachable-by-design PRE_VERSION guard) + declined the `$RUNNER_TEMP` suggestion (each matrix function runs on its own runner VM, pre/post are distinct sequential files → no race). **Codex GATE-4 caught a real MEDIUM:** the strict flag fired on ANY changed file under a function dir, so a **test-only change** (the 2 `*.test.ts` files that live in function dirs, which are NOT bundled) would false-fail the strict no-op check. **Fixed by making strictness per-function:** detect-changes now emits a `strict_functions` JSON array — a function is strict only if a NON-test file in its dir changed (the prompt `*.md` files DO count: they ship in the bundle via `new URL('./prompts/…', import.meta.url)`); the verify step derives its own strictness by jq membership. Per-function classification + membership re-validated under bash (test-only→lenient, source/`.md`→strict, mixed→strict).
- C40 (out-of-repo) done the same session: archive-split the 3 biggest closed-initiative journals into slim active + full `*_archive` files via a verify-gated Workflow (overallPass; adversarial verify caught + fixed 3 missed deferred items).
- **Next:** confirm the per-function `strict_functions` wiring via one more TEST dispatch (lenient → WARN/PASS) → let PR #537 CI settle → **user's merge go** (PROD-pipeline workflow; never merge without it) → then the user-gated PROD edge approval is a separate step (this PR only adds the verify; it does not deploy anything to PROD).
