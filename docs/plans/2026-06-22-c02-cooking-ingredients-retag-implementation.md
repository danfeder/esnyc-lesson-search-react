...
> **⚠️ AS-BUILT (Session 23, PR #549, `d297f64`) — the "re-canonicalize drift in the same migration before `ADD CONSTRAINT`" step below is SUPERSEDED by a user-verdict NOT VALID approach.** The drift re-census WAS run (read-only TEST+PROD) and found 18 pre-existing off-vocab rows P3 never re-tagged (11 live corpus-exclusions + 7 retired). Rather than mutate them, the constraints were added **`NOT VALID`** (enforce on future writes, grandfather the 18, zero data mutation; future `VALIDATE CONSTRAINT` after the deletion cleanup). See design §4 Q9 (NOT VALID amendment) + the status doc. The 4b.1 step below is kept for history; the as-built migration omits the snapshot/re-canonicalize machinery.
