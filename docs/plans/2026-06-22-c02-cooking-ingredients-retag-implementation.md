...
**P2′.8 — Per-field ship policy + floor-retention cooking + field-isolated validation (D-P11) — ⚠️ RUN BEFORE the P2′.7 freeze.** Scripts-only; rides the P2′ branch. Decision (a) locked Session 14 → the full run ships a **different source per field**, so the harness must materialize that and stop one field's failure from killing the other.
   - **Per-field ship materialization.** A new explicit `materializeC02Ship(record, floor)` (or equivalent) that produces the **apply input** per field: `main_ingredients` = the deterministic floor over the row's existing tags (**floor-only**, ignore the LLM ingredient decision); `cooking_skills` = **floor ∪ `finalC02.cooking_skills`** (floor-retention — the LLM may KEEP/ADD but never drops a floored skill). This is the canonical per-field output `generate-diff-report`/`prepare-apply` consume (supersedes the carry-forward "thread `finalC02` into the diff" — it's now a per-field-sourced ship object, NOT raw `finalC02`).
   - **Field-isolated run-time validation.** In `run-retag.ts`/`reconcile.ts`, validate + reconcile the two C02 fields **independently** so an off-vocab value in one (e.g. an off-vocab ingredient ADD) drops/fails only THAT field and never discards the other field's valid decision. The pilot saw 4 valid-cooking lessons lose all cooking output to an off-vocab ingredient (whole-record coupling). Decide the per-field failure policy (drop the offending value vs fail-that-field-to-floor) + **report** any field that fell back.
   - *Test:* ship materializer emits floor-only ingredients (an LLM ingredient ADD is absent from the ship output) + floor-retention cooking (a floored skill the LLM dropped IS present; an LLM cooking ADD IS present); an off-vocab ingredient ADD fails ingredients-only and leaves cooking intact (regression test reconstructing the 4 pilot records); floor-retention cooking ⊇ the floor for every row (no clean-core data loss).
   - *Commit:* `feat(c02): per-field ship policy — floor-only ingredients + floor-retention cooking + field-isolated validation (P2′.8)`

**P2′.7 — Fresh-25 label + final greenlight (D-P2) — USER-GATED.** **Freeze** the prompt/reconciler + **the per-field ship policy (P2′.8)** + the gate thresholds (no re-tuning after this point); commit the disjoint 25 ids (P2′.5); label them **independently** (two body-only derivations that see neither model output nor anchors + user adjudication — the Q6 protocol); run the frozen harness over the 25 + **materialize the per-field ship output** for scoring. **Acceptance contract:** the 25 is the anti-overfit CANARY — it must hold **aggregate** precision/recall **on the SHIPPED per-field output** (floor-only ingredients + floor-retention cooking, NOT raw `finalC02`) + the named cooking sentinels (Tasting/Kitchen-safety); the **per-value 46-specific gating stays on the 69** (25 rows can't measure 46 values — D-P2). Present the greenlight + cost projection. **Do NOT start P3 without explicit approval.**
