...
**P2′.7 — Fresh-25 label + final greenlight (D-P2) — USER-GATED.** **Freeze** the prompt/reconciler **and the gate thresholds** (no re-tuning after this point); commit the disjoint 25 ids (P2′.5); label them **independently** (two body-only derivations that see neither model output nor anchors + user adjudication — the Q6 protocol); run the frozen harness over the 25. **Acceptance contract:** the 25 is the anti-overfit CANARY — it must hold **aggregate** precision/recall + the named sentinels (Tasting/Kitchen-safety/Sweeteners); the **per-value 46-specific gating stays on the 69** (25 rows can't measure 46 values — D-P2). Present the greenlight + cost projection. **Do NOT start P3 without explicit approval.**
