...
**Per-PR ritual status:** pre-push Claude code-reviewer = clean (all 7 grade-router invariants hold; no critical/important bugs; 2 non-defects: dismiss-rewrite already verified via Header `localQuery`←`filters.query` sync + passing E2E; raw-query-pill coexisting with the auto chip is the locked design). GATE-3 Codex adversarial review running (`b7y8v1gyk`) — triage before push. NEXT: triage Codex → `npm run type-check && npm run lint` → `git push -u origin feat/search-g2-frontend` → `gh pr create` → wait for external bots → four-surface triage → rebuttal-pass → fix-ups (re-run `eval:search` after any search-code fix-up) → round-cap after 2.
...
- **Pre-push review (Session 5):** Claude `feature-dev:code-reviewer` on `git diff main...HEAD` = clean (7 invariants hold, no critical/important bugs, 2 non-defects rebutted). GATE-3 Codex adversarial review dispatched in background (`b7y8v1gyk`).

NEXT: triage GATE-3 Codex findings → push `feat/search-g2-frontend` → `gh pr create` → four-surface bot triage → fix-ups (re-run `eval:search` on any search-code fix-up). S1 ships via Netlify (no DB change → no TEST-DB-before-merge gate). After S1 merges: S2 (G3 SEL field indexing — first migration PR of this track).
