...
NEXT: push `feat/search-g2-frontend` → `gh pr create` → wait for external bots → four-surface bot triage → fix-ups (re-run `eval:search` on any search-code fix-up) → round-cap after 2. S1 ships via Netlify (no DB change → no TEST-DB-before-merge gate). After S1 merges: S2 (G3 SEL field indexing — first migration PR of this track).
