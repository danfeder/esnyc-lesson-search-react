...
**Locked decisions (investigation Q2; scoring model upgraded 2026-06-18 after the S0.2 dual adversarial review — Claude 4-lens fan-out + independent Codex, both against the live TEST corpus):**
- **~32 queries** in a committed `scripts/search-eval/queries.json`, every query provenance-traceable (no usage logs ⇒ provenance is the only defense against author bias): reproduced gap cases (`decay`, `decomposition`, `compost`, `compost lesson for 3rd grade`, G2 false-positive guards like `three sisters garden`), everyday→official phrasings incl. **discriminating** SEL/CC values, obvious teacher intents, **cuisine/culture coverage** (`mexican food` — Cultural Heritage is a first-class filter and was entirely missing), seasonal (`apple`), control/regression incl. the **multi-meaningful-term** sentinel (`compost worms soil`), and a small **robustness pack** (grade-wording variants `3rd`/`third graders`/`K-2`, one typo, one extra cuisine `kimchi`) to stress the grade router + typo expansion.
- **"Ideal result" = engine-independent, human-confirmed gold, scored per a per-query SCORING FAMILY** (NOT one flat metric). Both review passes proved a single hit-rate@10-vs-flat-id-list design would give **false readings in ≥4 ways**; the upgraded model below is the locked replacement. Gold is built collaboratively and **the product owner signs off** the frozen sets.
  - **Two-tier, cluster-aware gold.** Each gold entry is a **cluster** of equivalent `lesson_id`s (exact-duplicate twins → one cluster; *either* twin satisfies it — prevents duplicate-flooding from inflating recall and prevents future de-dup from breaking the gold). Within a query, clusters are **primary** (bullseye; counted) or **acceptable** (reasonable-adjacent; never penalized, not required). The two tiers resolve the genuine Claude-vs-Codex relevance disagreements (e.g. pollinator *life-cycle* lessons → acceptable, pollination-*mechanism* lessons → primary) without full graded nDCG (still skipped — too costly).
  - **Scoring families (per query):** (1) **frozen-recall** — `recall@10` vs primary clusters + `top1` + `MRR` (for small exhaustive answer sets: decomposition bridge, knife-teaching, photosynthesis, pickling). (2) **frozen-precision** — `precision@10` counting primary+acceptable as relevant + `top1`/`MRR` vs primary (for broad-theme pools >10: pollinators, three sisters, compost-control). (3) **predicate** — a concrete, re-tag-survivable, snapshot-pinned predicate with sampled FP/FN (e.g. `tomato`, `mexican food`). (4) **g3-isolation** — see below. (5) **sentinel** — `compost worms soil`: a stability alarm (Jaccard vs a stored snapshot + total_count band), **excluded from the quality score** (freezing today's top-10 as *gold* would grade against current bad behavior — engine-independence violation). (6) **control-maxcount** — `garden`, G2 cases: `overBroad`/`maxTotalCount` guard.
  - **Every relevance query also reports `unique-content precision@10`** (distinct clusters in top-10 / 10) as a duplicate-flooding guard.
  - **G2 cases are scored on the NORMALIZED call, not raw.** The G2 fix is frontend (`parseSearchQuery`), upstream of the RPC — so the harness applies `parseSearchQuery` and scores the cleaned `search_query` + routed `filter_grade_levels` + a `maxTotalCount` guard. Baseline (S0, before the module exists) records the raw explosion; S1 records the normalized fix. Each G2 query freezes its expected normalized call.
  - **G3 (typed-field indexing) is scored by rank MOVEMENT, not top-10 presence.** Measurement set = lessons carrying the SEL/CC tag but with **zero lexical mention** of the phrase anywhere in title/summary/body (the *only* lessons that move solely because the typed array got indexed): **RDM=29, Self-management=42, Social Justice=25** (TEST, retired-excluded). Metrics: `isolationHits@50`, first-isolation rank/MRR, and median/best **rank movement** baseline→after (top-10 is a *secondary* UX signal — after indexing, lexical matches can still outrank tag-only lessons, so top-10 recall would falsely read "didn't work"). These tags are **human-audited** before being treated as relevance truth; concept/tag membership is a *candidate generator + provenance*, never binary gold alone.
  - **A tolerance band** on regression, not brittle zero-jitter; all counts/gold pinned to a dated TEST snapshot.
- **Lives as** a standalone read-only **TypeScript** script run via **`npx tsx`** (`scripts/search-eval/run-search-eval.ts` — the repo convention for scripts, e.g. `npx tsx scripts/backfill-embeddings.ts`; a plain `.mjs`/`node` script CANNOT import the TS `parseSearchQuery` module S1 needs, so it must be `.ts` + tsx), anon key, its own `assertReadOnly` guard — NOT a mirror of the local-only/write-based heritage harness — + `queries.json` + committed `baseline.json` + scorecards, exposed as `npm run eval:search`. **GATE-1 fix:** add `scripts/search-eval/**/*.ts` to `tsconfig.scripts.json` `include` so the harness + helpers are type-checked. **Not a vitest** (needs the ~767-row DB; CI has no live Supabase — same precedent as `test:rls`). Pure helpers (parsing, metric math) ARE vitest-tested. The ranking scorer is **written fresh** — `scripts/lib/evalMetrics.ts` `computeMetrics` is a multi-label classification metric (no rank/k concept) and cannot be reused.
...
| — | **PR-E rider (dead code)** | Folded into existing `feat/pr6e-stage2-cleanup`: `DROP FUNCTION generate_lesson_search_vector(10-arg)` + revoke grants; delete `supabase/functions/search-lessons/` **AND run `supabase functions delete search-lessons` for TEST+PROD** (GATE-1 fix: it is deployed ACTIVE v22 — the deploy workflow only discovers+deploys existing dirs and never undeploys, so dir deletion alone leaves it live) + MCP-verify removal | NEW addition to PR-E (E1/E2/E3 don't currently scope it). Gated after C2 AND S2 PROD-verified. |
