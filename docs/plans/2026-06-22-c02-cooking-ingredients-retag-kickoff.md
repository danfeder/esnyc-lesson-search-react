...
**⚠️ THE P2 PILOT FAILED (Session 8, 2026-06-23) — the initiative is at a METHOD PIVOT, not on the original P2→P3 track.** P1 is MERGED (`a5ff8a9`); P2.1 prep + P2.2 gold key are done on `feat/c02-pilot`; **P2.3/P2.4 ran and BOTH `claude-opus-4-8` (stronger) and the `claude-opus-4-7` fallback failed all 4 gates by over-tagging** (root cause = the harness is effectively a *blind* full-LLM re-read; the floor's R7/R8 rules never fire on the model's canonical output, so nothing anchors the clean core). Codex cross-exam confirmed the diagnosis and de-risked the fix. **The design doc Status still says `LOCKED` but now carries a PILOT-OUTCOME banner re-opening §3/§4-Q3/Q5/Q6 + the apply scope — read that banner; do NOT treat the method as settled.** **The next session is a METHOD-PIVOT DESIGN session** (not a build, not a re-run): re-lock the new approach — **floor-first reconciliation in code** + **anchored prompt + negative few-shots** + **C02-only apply scope** (the generic apply rewrites ALL metadata fields — a real P3 blast-radius risk) + **per-value precision gates** + a **fresh held-out test slice** (the 69-key is now tuning data, contaminated for the final greenlight) — then build. **The status file's Current State header is the load-bearing orientation; read it + the two pilot scorecards (`scripts/stage2-retag/artifacts/c02-pilot-scorecard.opus-4-{8,7}.md`) + `c02-filter-usefulness.md` before deciding anything.** Proxy/runbook note: CLIProxyAPI v7.2.33's Anthropic base URL is `http://127.0.0.1:8317` (the old `…/api/provider/anthropic` path is gone); both runs cost ~$3.7–3.9 at ~$0.055/lesson. Follow the SESSION-START RITUAL + EXECUTION MODE above: orient, confirm with the user, then proceed. **Trust git + the status file over any stale line here.**
