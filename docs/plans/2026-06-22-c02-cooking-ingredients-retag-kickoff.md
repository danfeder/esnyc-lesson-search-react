...
Phased delivery (see the design doc §8 + impl plan):
  ...
  - **P2:** pilot — **single Opus-4.8 run** (Sonnet bake-off SKIPPED per user decision 2026-06-23, `project_c02_p2_opus_only`) → 4-gate greenlight (artifacts)
...
- **Method = anchored verify-and-diff** (PIVOT — design **§3·PIVOT D-P1..D-P10**, re-locked Session 9; the old "hybrid-floor full LLM re-read" was, as built, a *blind* re-read that over-tagged — DEAD). The LLM sees the body **+ the current provenance-annotated tags** and proposes **KEEP/DROP/ADD**; **one canonical floor** + a code-level **`reconcile.ts`** (never append-only) merge floor + LLM into `finalC02`; **per-value gates** with a **recomputed baseline**; **C02-only apply scope** + concurrency guard + targeted rollback. Two user-verdicts: **D-P1** gentle-leash-then-measure (no add-lock in pilot-1), **D-P2** fresh-25 anti-overfit canary.
...
**✅ P3 + P4a SHIPPED TO PROD; ⏳ P4b BUILT + TEST-VERIFIED — PR #549 OPEN, awaiting PROD approval (USER-GATED). This is the LAST phase — once #549 merges + PROD-verifies, the initiative is DONE (do the close retrospective).** Shipped: P3 apply (#544 `07e1a4e`, data canonical on PROD), P4a app-layer enforcement (#547 `8513b18` — frontend dropdowns + Zod enums + `superRefine` + edge mirror + `canonicalizeReviewMetadata`; deployed + LIVE), manifest FROZEN. **P4b (Session 23):** enums.json registration (`b8f8090`) + `20260626000000_c02_enforce_check.sql` (`d297f64`) = two table-wide CHECKs `valid_cooking_skills`/`valid_main_ingredients`, **NOT VALID** (user verdict 2026-06-26 — a read-only PROD drift census found 18 out-of-scope grandfathered rows [11 live corpus-exclusions + 7 retired] a plain CHECK would reject; zero data mutation; future `VALIDATE` after the deletion cleanup). PR #549 = TEST-verified ALL GREEN + GATE 2/3 + claude-review all clean (4-surface triage done, **0 actionable**). **The NEXT session:** orient (status **Current State header** is load-bearing — read it; **trust git over any stale line here**) → confirm with the user → **finish the P4b PR cycle**: push the local Session-23 docs commit if still unpushed → user-gated **PROD approval** (merge #549 → `migrate-production.yml` → approve) → **mandatory PROD MCP verify** (both constraints present + `convalidated=false` + live def byte-identical 23/70 + behavior off-vocab-reject/canonical-pass) → **initiative-close retrospective** (lift out-of-scope follow-ups to project memory: the future `VALIDATE CONSTRAINT`, the corpus-exclusion deletion track, drop `c02_retag_*` snapshot tables, optional live-DB enums↔CHECK sync test, collapse dup `setsEqual`/`mulberry32`; promote process learnings; MEMORY.md hygiene + flip the project memory to closed; archive the P4 PR cycle). **Do NOT re-run/re-apply P3 or P4a; do NOT touch `20260625000000_c02_retag_apply.sql`; do NOT re-open the frozen `c02-vocab.json`.** Deploy choreography = design **§4 Q9 (NOT VALID amendment) + §7**; impl detail = impl plan **P4b**. Follow the SESSION-START RITUAL + EXECUTION MODE above.
