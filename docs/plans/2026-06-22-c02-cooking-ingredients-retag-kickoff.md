...
**✅ P3 SHIPPED TO PROD (Session 21) — next session = P4 ENFORCEMENT (not a data session, not a re-run).** The pilot (P2′) and the apply (P3) are DONE: PR #543 (P2′) + PR #544 (P3, `07e1a4e`) merged; the apply migration is **applied to PROD + MCP-verified** (450 updated / 0 skipped / canonical / 0 off-vocab / 0 lockstep; 11 residual = intentional corpus-exclusions). **The LLM run and the apply are DONE — do NOT re-run or re-apply; do NOT touch the applied migration `20260625000000_c02_retag_apply.sql`.** The next session: orient (the status **Current State header** is the load-bearing piece — read it) → confirm with the user → **FREEZE the manifest first** (the run surfaced no missing high-frequency food → no `c02-vocab.json` change, just confirm + lock) → build **P4a** (`feat/c02-enforce`, frontend: non-creatable dropdowns in `filterDefinitions.ts` + `ReviewDetail.tsx`, `CookingSkillsEnum`/`MainIngredientsEnum` + specific→group `superRefine` across all **4** Zod schemas incl. the edge mirror, TDD) → merge → frontend auto-deploys → THEN **P4b** (`feat/c02-check`, the `valid_cooking_skills`+`valid_main_ingredients` CHECK migration, AFTER a fresh TEST+PROD drift re-census, GATE 2 Codex). **Expand/contract: P4a and P4b are SEPARATE PRs/approvals — never bundle them (§4 Q9).** All derive byte-identically from the frozen manifest. Impl detail = impl plan **P4a/P4b**; deploy choreography = design **§4 Q9 + §7**. Follow the SESSION-START RITUAL + EXECUTION MODE above. **Trust git + the status file over any stale line here.** _(Deferred cleanups in the status doc out-of-scope list: drop the retained `c02_retag_*` snapshot tables once PROD-stable; collapse dup `setsEqual`/`mulberry32`.)_
