...
8. **Pre-delete reference scan for the 3 ghost IDs — SPLIT BY ENFORCEMENT** (GATE A's key correction: an unenforced text ref does NOT block a `DELETE`, so "zero refs" must be *data-queried*, never inferred from FK-absence). `[evidence-lockable]` (escalate `[user-verdict]` if any ref found). GATE-A-grounded all-clear on TEST for the 3 ghosts; **re-run on PROD with verbatim IDs** (`feedback_verbatim_identifiers_in_probes`):
   ...
   - **CASCADE-child precondition:** the 2 CASCADE FKs (`bookmarks.lesson_id`, `canonical_lessons.duplicate_id`) verified 0 child rows for the 3 ghosts → a parent-row `.rollback` is whole. If any CASCADE child appears on PROD, escalate `[user-verdict]` (rollback can't restore cascade-deleted children). <!-- TBD Session 1 -->
...
10. **C49 — drop the dead `filter_lesson_format` param.** `[evidence-lockable]` — GATE-A-grounded: a caller DOES reference it (`src/hooks/useLessonSearch.ts:150` sends `filter_lesson_format: undefined`; `src/types/database.types.ts:1444` types it), **BUT `JSON.stringify` strips `undefined`-valued keys so PostgREST never receives the arg** (verified on TEST: omitting the param returns rows; the W1b RPC already removed its WHERE clause — inert-but-present). So the RPC param drop is **safe to fold into PR2's `DROP+CREATE`**, and C49 should ALSO delete the now-dead `useLessonSearch.ts:150` line + regenerate `database.types.ts` (so the §7 grep comes back truly clean). The reverse-PGRST202 split is only needed if a caller sent a *real* value — not the case here. Lock the expand/contract decision in Session 1. <!-- TBD Session 1 -->

**PR 3 — C02 (vocab canonicalization):**
11. **The locked cooking_skills / main_ingredients canonical vocab + the messy→canonical mapping.** `[evidence-lockable]` — load the vocab worksheet (memory: "Vocab worksheet — DONE `93b929e`"); confirm it covers both fields; derive the row-level UPDATE mapping via the proven rebuild pipeline. <!-- TBD Session 1 -->
12. **Rollback-snapshot table shape + how to scope the UPDATE (which rows, which JSONB keys).** `[evidence-lockable]` — default: snapshot affected rows' `metadata` (or the two keys) into a rollback table the migration creates; UPDATE only rows whose values change. <!-- TBD Session 1 -->
13. **Both fields in one PR, or stage cooking_skills then main_ingredients?** `[user-verdict]` — default leaning: one PR (the rebuild "two-rounds" note suggests they're coupled), but allow a split if the mapping volume is large. <!-- TBD Session 1 -->
...
- **What changes:** bulk UPDATE of `cooking_skills` / `main_ingredients` metadata values across affected lessons via the proven canonicalization pipeline.
- **Hazards:** medium — touches many rows' `metadata` JSONB, but these two fields are **not sidebar filter facets** (verified: `cookingSkills` `filterDefinitions.ts:271` + `mainIngredients` `:174` live in `METADATA_CONFIGS`, explicitly commented "NOT search filters"), so no facet-count/filter-UI regression. Snapshot affected `metadata` to a rollback table first (Q12). Re-verify post-update value distribution on TEST.
- **Mechanism:** one migration + rollback table. Reuse the metadata-rebuild canonicalization machinery (vocab-locked, pipeline-proven).
