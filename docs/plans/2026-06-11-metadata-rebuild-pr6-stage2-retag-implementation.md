...
- **E2c — Legacy review-row canonicalizer (GATE-1 Codex finding, ACCEPTED + PROD-confirmed Session 21).** `submission_reviews.tagged_metadata` (113 rows, ALL `decision=approve_new` / submission `status=approved`) stores **LEGACY SLUG forms** for 4 of the 6 fields (`cookingMethods` + `observancesHolidays` are already canonical there, no map needed). `ReviewDetail.tsx:431` **unconditionally restores** `tagged_metadata` into the form whenever a review row exists, so once E2b closes `reviewFormPayloadSchema`, reopening one of these legacy rows renders selections deselected AND **rejects re-save** (`reviewFormPayloadSchema.safeParse` at `ReviewDetail.tsx:533` + the edge `complete-review:118`). **Fix (frontend, NO historical-data rewrite — preserves the forensic review records):** add a shared `canonicalizeReviewMetadata(meta)` util mapping legacy→canonical for the 6 fields, applied BOTH (a) on LOAD wrapping `reAddActivityTypeSuffix` at `ReviewDetail.tsx:431` (form displays correct selections on reopen) AND (b) on SAVE immediately before `reviewFormPayloadSchema.safeParse` at `ReviewDetail.tsx:533` (residual legacy values normalize before the now-closed schema validates). The edge receives the already-canonicalized payload, so no edge change required (optional belt-and-braces: canonicalize before the edge safeParse too). **Verbatim legacy slug→canonical maps (PROD-censused Session 21 — but re-verify each slug against the live `submission_reviews` census + `smaller-fields.vocab.json` before trusting; build the map defensively so an UNMAPPED legacy slug passes through unchanged rather than throwing):**
  - `academicIntegration`: `math`→`Math`, `science`→`Science`, `literacy-ela`→`Literacy/ELA`, `social-studies`→`Social Studies`, `health`→`Health`, `arts`→`Arts`
  - `socialEmotionalLearning`: `relationship-skills`→`Relationship skills`, `self-awareness`→`Self-awareness`, `responsible-decision-making`→`Responsible decision-making`, `self-management`→`Self-management`, `social-awareness`→`Social awareness`
  - `coreCompetencies`: `environmental-stewardship`→`Environmental and Community Stewardship`, `social-justice`→`Social Justice`, `social-emotional`→`Social-Emotional Intelligence`, `garden-skills`→`Garden Skills and Related Academic Content`, `kitchen-skills`→`Kitchen Skills and Related Academic Content`, `culturally-responsive`→`Culturally Responsive Education`
  - `gardenSkills`: the PRE-E2b `filterDefinitions.ts` garden `value`(slug)→`label`(Title) map (capture it BEFORE E2b flips those to value=Title), e.g. `composting`→`Composting`, `sensory-exploration`→`Sensory exploration`, `beneficial-insect-id`→`Beneficial insect identification`, `soil-preparation`→`Soil preparation and care`, `tool-maintenance`→`Tool use and maintenance`, `garden-exploration`→`Garden exploration`, `preservation`→`Preservation techniques`, `watering-techniques`→`Watering techniques` (full 22-entry slug→label set lives in the current filterDefinitions garden options)
  - `cookingMethods` / `observancesHolidays`: already canonical in tagged_metadata; include defensive folds anyway (`basic-prep-only`/`no-cook`→`basic-prep`; `End of year`→`End of year celebrations`; `Earth month`→`Earth Month`) since they're cheap and the data could surprise.
  - **Tests (Codex rec):** reopen+save fixtures using LEGACY slug values for all 4 slug fields (not only canonical), asserting they canonicalize on load AND pass the closed `reviewFormPayloadSchema` on save. Plus a fixture proving an already-canonical value round-trips unchanged.
- **E2 verification:** `npm run generate:enums` (clean regen, 6 new keys); `npx vitest run src/types/edgeSharedSchemas.equivalence.test.ts src/types/generated/enums.json.test.ts`; `npm run test` (full suite); `npm run type-check && npm run lint`. Confirm no remaining `z.array(z.string())` open-form for the 6 fields in any of the 4 schema copies; confirm filterDefinitions value-forms === smaller-fields.vocab.json for all 6; confirm a legacy-slug `tagged_metadata` fixture reopens + re-saves green.
