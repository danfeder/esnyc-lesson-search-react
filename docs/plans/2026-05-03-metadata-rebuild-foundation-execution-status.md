...
**Last updated:** 2026-05-04 — Session 7 complete (PR 1 Task 1.3 — lessonFormat removal SQL migration shipped: view + 3 active RPCs + trigger + 2 indexes + JSONB key strip + column drop)
**Current PR:** PR 1 in flight (multi-task; opens after Task 1.6 lands)
**Current task:** PR 1 Task 1.3b — lessonFormat removal frontend + edge function sweep (~30 file surfaces; types/index.ts, IntCard/IntListRow/IntSidebar, filterDefinitions/filterUtils, ReviewDetail/ReviewMetadataForm, useLessonSearch/useLessonSuggestions, facetCounts, search-helpers, 6 test fixture files, scripts/migrate-metadata-to-columns + analyze-duplicates × 2, plus the Session-4 Zod-scaffold 13 refs across 5 files)
**Branch:** `feat/metadata-foundation-schema` (local, not pushed)
**Last commit on feature branch:** `35cac25` — feat(metadata-foundation): drop lesson_format coordinated removal. Session 7 will add a status-doc commit at session end.
...
(none — Session 7 wrapped Task 1.3 SQL migration; next session opens Task 1.3b — lessonFormat removal frontend + edge function sweep)
