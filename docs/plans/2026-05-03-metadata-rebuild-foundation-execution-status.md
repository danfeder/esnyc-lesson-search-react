...
- **PR #487** (open 2026-05-11): Stage 1 heritage worksheet European cluster per-value entries (14 entries: 1 cluster root European 53 + 2 sub-regions [Mediterranean 39 v3, Eastern European 3 v3] + 5 country-specifics with corpus [Italian 24 v3, Spanish 5 v3 multi-parent, Ukrainian 3 v3, Greek 2 v3, Russian 1 v3] + 2 v3-canonical-corpus-absent [French 0, Polish 0] per §12.18 Dominican precedent + 1 NEW [Irish 2] + 3 kebab-case drift); §9.2 Spanish multi-parent resolved to Mediterranean primary (European cluster) with Latin American flagged; 3 Opus corpus-read `<details>` blocks integrated; 14 EUR-NN audit signals appended (largest single-cluster audit-signal cohort to date). Session 68. Squash hash + merge time TBD.
