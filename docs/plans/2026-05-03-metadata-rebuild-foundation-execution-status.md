...
**Last updated:** 2026-05-04 — Session 9 complete (PR 1 Task 1.4a — Activity Type expand to 5 with `craft-only` slug shipped: 2 files / +87)
**Current PR:** PR 1 in flight (multi-task; opens after Task 1.6 lands)
**Current task:** PR 1 Task 1.4b — new "Lesson Type" tag-based sidebar section (requires `search_lessons` RPC DROP+CREATE to add `filter_tags text[] DEFAULT NULL` parameter + ~7 frontend wiring edits). Per impl plan §1.4b.
**Branch:** `feat/metadata-foundation-schema` (local, not pushed)
**Last commit on feature branch:** `29881b6` — feat(metadata-foundation): activity_type filter — add craft-only (Task 1.4a). Session 9 will add a status-doc commit at session end.
...
(none — Session 9 wrapped Task 1.4a; next session opens Task 1.4b — new "Lesson Type" tag-based sidebar section per impl plan §1.4b. Task 1.4b is bigger scope than 1.4a: needs `search_lessons` RPC DROP+CREATE to add `filter_tags text[] DEFAULT NULL` parameter + ~7 frontend wiring edits across types/index.ts, searchStore, filterDefinitions, facetCounts, filterUtils, useLessonSearch, and possibly ScreenReaderAnnouncer.)
