...
(none — PR 2 shipped; PR 3 to start in next session)
...
**PR 3 — Reviewer flow redesign** (full Section 6.1 banner, intent-aware reviewer state machine, picker integration on the reviewer side).

Branch from `main`: `feat/phase-8b-reviewer-flow`. Implementation plan starting at "## PR 3 — Reviewer flow" (line 1522 in `docs/plans/2026-04-27-phase-8b-approve-update-redesign-implementation.md`).

**Watch out for:**
- The deploy-edge-functions workflow "deploy reported success but source didn't change" issue (Phase 7c, MEMORY.md) does NOT apply to PR 3 unless PR 3 modifies an edge function. Confirm at task-time.
- The PR 2 banner in `ReviewDetail.tsx` (commit `41c9c2e` on origin/main) is the throwaway version — PR 3 replaces it with the enriched Section 6.1 banner (Tailwind classes, title lookup, reviewer-actionable copy). Delete the PR 2 banner block; don't try to extend it.
- Several PR 2 follow-ups (DRY auth hook, DRY badge constant, DRY URL regex, `<fieldset disabled>` for Step 2, keyboard nav on picker) are natural to fold into PR 3 since it touches the same file tree. Decide at PR-3 design time which to bundle vs defer.
