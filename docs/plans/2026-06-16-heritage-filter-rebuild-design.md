...
### 3.7 Facet-count coherence (C1.6 decision — LOCKED 2026-06-16, user verdict)
A C1.6 investigation (read-only, Session 15) found the heritage count badges are **already broken on prod** (not a C1.4/C1.5 regression): `counts.culturalHeritage` is keyed verbatim by the **stored Title-Case labels** (`facetCounts.ts` tally), but the section component looks up by **kebab slug** (`countFor(node.value)`) → every badge resolves to `0`/blank. This pre-dates C1.4 (old options were already slugs while storage was already labels) and is independent of the sparse local seed. So the impl plan's premise "for C1 current counts are still accurate" is **false for heritage**.
- **DECISION (user):** **fix the slug↔label count keying AND make parent counts expansion-aware.** A parent's badge = the count of **distinct lessons** matching that node **plus all transitive descendants** (incl. internal-tier traditions, e.g. a `Soul Food` lesson credits `African American` and `Indigenous and Diaspora`). This stays coherent **now** and **after C2** (specific-only storage), so the count logic is written once and survives the data apply — consistent with the Option-B "clean end-state" choice.
- **Algorithm (avoid the double-count trap):** the count must be over DISTINCT lessons. Per lesson, normalize each stored label → slug, expand each slug to `{self ∪ all ancestors}` (slug space), **union into one Set per lesson**, then increment each slug's bucket once. Do NOT sum descendant counts (that double-counts a lesson tagged both `Chinese` and `Japanese` toward `Asian`). The result map is **slug-keyed**, so the component's `countFor(node.value)` works unchanged.
- **Maps needed (full tree, incl. internal):** a label→slug normalizer and a slug→ancestors map covering the FULL 71-node hierarchy, NOT just the UI `top`+`sub` options (a lesson can store an `internal`-tier label). Source these from the generator (preferred: extend `generate-heritage-hierarchy.ts` to ALSO emit a committed ancestry/alias map artifact — keep existing outputs byte-identical, extend the drift guard) or from `cultural-heritage.vocab.json`. `getCultureDescendantValues` (C1.4) walks DOWN and is the wrong direction / double-count-prone for the per-lesson tally — do not reuse it naively here.
- **Out of scope of this track:** `activityType` has the same class of slug-vs-label count bug (`cooking-only` slug vs `Cooking` stored) — logged as a separate follow-up, not fixed here.
