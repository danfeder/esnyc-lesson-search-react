...
- **Pre-existing OUTAGE CONFIRMED:** `supabase.functions.invoke('send-email')` from a service-role client inside a deployed edge fn **silently fails to deliver** — triggered user-management bulk-deactivate (200) → its `account-deactivated` invoke produced **NO send-email log entry**, while a direct call did. Matches `complete-review:304-309`'s empirical comment. So the **8 invoke-based send-email calls** (user-management ×4 `:543/566/638/791`, password-reset ×2 `:167/243`, invitation-management ×2 `:253/379`) have NOT been sending. Only `complete-review` (raw fetch) + the frontend (browser) actually deliver.
- **LOCKED per-type matrix:** service-role-only (constant-time `Bearer SERVICE_ROLE_KEY`, mirror `extract-google-doc:42-60`) = `{password-reset, password-changed, role-changed, account-deactivated, account-reactivated, submission-approved, submission-needs-revision, submission-rejected}` (8); admin-JWT/browser = `{invitation, welcome}` (2). **Gate shape:** `isServiceRole` → allow any type; else if type ∈ service-role-only → reject; else (invitation/welcome) → require admin user-JWT (existing block); unknown type → reject.
- **Option B-full (user-approved):** also switch the **6 LIVE** invoke sites (user-management ×4 + password-reset ×2) → raw fetch (mirror `complete-review:310-322`, `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` + `apikey`) so they reliably authenticate AND the silent outage is fixed. `invitation-management` ×2 LEFT (dead C128 orphan; sends admin-JWT-only `invitation` → raw-fetch wouldn't help) — documented, not edited.
