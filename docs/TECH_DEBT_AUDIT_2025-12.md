...
- [ ] Delete `docs/development/FIXES_NEEDED.md`
- [ ] Remove unused exports (`withProtection`, `ErrorBoundaryTest`)
- [ ] Remove unused npm packages (`tiktoken`, `csv-parse`)
- [ ] Move `openai` to devDependencies
- [ ] Run `npm audit fix`
- [ ] Condense Algolia section in `src/lib/CLAUDE.md`

### Phase 2: Dead Code Cleanup (2-3 days)

- [ ] Remove unused functions in `featureFlags.ts`
- [ ] Remove unused functions in `virtualization.ts`
- [ ] Remove `isUniqueConstraintError()` from `errorHandling.ts`
