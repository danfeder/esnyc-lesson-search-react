-- Apply NULL filter fix to search_lessons function
-- Run this in Supabase SQL Editor

-- First check current state
SELECT COUNT(*) as total_lessons FROM lessons;

-- Drop and recreate the function with fixed NULL handling
...
-- Test the function with no filters (should return all 832 lessons)
SELECT 
    COUNT(*) as result_count,
    (SELECT COUNT(*) FROM lessons) as total_lessons
FROM search_lessons();

-- Test with first 5 results
SELECT 
    lesson_id,
    title,
    total_count
