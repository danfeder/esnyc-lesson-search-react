...
SELECT DISTINCT
    metadata->>'locationRequirements' as stored_location,
    jsonb_typeof(metadata->'locationRequirements') as json_type
FROM lessons
WHERE metadata->>'locationRequirements' IS NOT NULL
LIMIT 10;

-- Fix the search function to handle location stored as JSON array string
...
GRANT EXECUTE ON FUNCTION search_lessons TO anon;

-- Test the fix
SELECT COUNT(*) as indoor_lessons
FROM search_lessons(filter_location := ARRAY['Indoor']::text[]);

-- Test multiple filters again
