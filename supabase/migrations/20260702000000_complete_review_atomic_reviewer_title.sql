-- =====================================================
-- Migration: 20260702000000_complete_review_atomic_reviewer_title.sql
-- (Dated 07-02: 20260701000000 was already taken by the taste-test carve-out
--  migration; migration versions must be unique.)
-- =====================================================
-- Description: T2b quick-wins — let the reviewer's edited Title win when
-- publishing/merging. The reviewer form now carries an editable "Title" field
-- (prefilled from the doc). Previously complete_review_atomic preferred
-- lesson_submissions.extracted_title (the raw Google Doc filename) over the
-- reviewer's edit, so a reviewer-corrected title was silently discarded.
--
-- CHANGE: flip ONLY the title precedence in the two lesson-write spots so the
-- reviewer's payload title (v_meta->>'title') is preferred:
--   * approve_new INSERT (was: extracted_title → v_meta.title → 'Untitled Lesson')
--       now: v_meta.title → extracted_title → 'Untitled Lesson'
--   * approve_update UPDATE (was: extracted_title → v_existing.title)
--       now: v_meta.title → extracted_title → v_existing.title
--     (Keeps v_existing.title as the final fallback rather than the INSERT
--      path's 'Untitled Lesson' — the merge path must never clobber an existing
--      lesson's title with a placeholder. Title is a required, prefilled field
--      on both approve decisions, so an empty v_meta.title cannot reach this via
--      the gated UI; this fallback only guards a direct/edge RPC call.)
--
-- Everything else — the summary line, the tags side-channel, the activity_type
-- array passthrough, the academicConcepts carry-forward, the metadata blob
-- build, the idempotency guard — is copied VERBATIM from the prior definition
-- (20260519000000_complete_review_atomic_tags_side_channel.sql), which is the
-- latest definition of this function (confirmed: 20260521000000 only defines
-- _flatten_academic_concepts + update_lesson_search_vector, not this RPC).
--
-- Function signature is UNCHANGED; CREATE OR REPLACE preserves grants. No DROP.
--
-- See: docs/plans/2026-07-01-brief-quickwins-patch.md (item F)
-- =====================================================
...
-- 20260519000000_complete_review_atomic_tags_side_channel.sql verbatim (which
-- prefers extracted_title over the reviewer-edited title). CREATE OR REPLACE
-- preserves grants; no GRANT statements are needed in the rollback. Function
-- signature is unchanged, so no DROP needed.
--
-- Side effect of revert: lessons published/merged after this migration keep
-- whatever title was written (the migration only changes the writer, not
-- at-rest data). Future approve_* would revert to preferring extracted_title.
