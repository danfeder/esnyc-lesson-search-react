-- Migration: Update duplicate resolution function with smart metadata merging (FIXED)
-- Date: 2025-01-31
-- Description: Implements actual metadata merging logic when resolving duplicates - Fixed version
  ...
  BEGIN
    ...
    IF p_merge_metadata THEN
      ...
      LOOP
        ...
        -- Handle cases where the field might not be an array
        BEGIN
          SELECT ARRAY(
            ...
            FROM (
              SELECT jsonb_array_elements_text(
                CASE 
                  WHEN jsonb_typeof(metadata->v_field_name) = 'array' 
                  THEN metadata->v_field_name
                  ELSE '[]'::jsonb
                END
              ) as value
              FROM lessons
        ...
        EXCEPTION
          WHEN OTHERS THEN
            -- If there's any error processing this field, skip it
            RAISE WARNING 'Could not merge field %: %', v_field_name, SQLERRM;
        END;
      END LOOP;
      ...
      LOOP
        IF v_merged_arrays->v_field_name IS NULL OR 
          ...
          SELECT metadata->>v_field_name INTO v_single_value
          FROM lessons
      ...
      EXCEPTION
        WHEN OTHERS THEN
          -- If tags merge fails, just log and continue
          RAISE WARNING 'Could not merge tags: %', SQLERRM;
      END;
    ...
    -- Update lesson_collections (handle the new structure)
    -- First, update any existing references
...
COMMENT ON FUNCTION resolve_duplicate_group IS 'Resolves duplicate lessons by archiving duplicates and optionally merging their metadata into the canonical lesson. Preserves the canonical lesson summary while merging array fields and filling in missing text fields. Fixed to handle non-array metadata fields gracefully.';