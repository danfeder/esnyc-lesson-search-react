...
CREATE OR REPLACE FUNCTION auth.is_reviewer_or_above() 
RETURNS BOOLEAN AS $$
DECLARE
  user_role TEXT;
  user_active BOOLEAN;
...
CREATE POLICY "Reviewers can view all profiles" ON user_profiles
  FOR SELECT
  USING (auth.is_reviewer_or_above());
...
CREATE POLICY "Admins can create invitations" ON user_invitations
  FOR INSERT
  WITH CHECK (auth.is_admin() AND invited_by = auth.uid());

CREATE POLICY "Admins can update invitations" ON user_invitations
  FOR UPDATE
  USING (auth.is_admin() AND invited_by = auth.uid())
  WITH CHECK (auth.is_admin() AND invited_by = auth.uid());
...
GRANT EXECUTE ON FUNCTION auth.is_reviewer_or_above() TO authenticated;